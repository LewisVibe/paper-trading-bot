...
- `python bot.py --show-current-research-state` is a compact saved-output-only terminal display helper for the multi-sleeve research state. It reads saved QQQ100 recovered-reference, high-growth stream, crypto stream, multi-sleeve portfolio, and crypto-review CSVs, shows the current QQQ100 reference, high-growth sleeve, crypto sleeve, multi-sleeve candidate, and safety state, labels missing saved outputs as `missing_saved_output`, does not refresh market data, does not approve preview promotion, does not approve execution, and does not connect strategies to Alpaca or paper orders.
