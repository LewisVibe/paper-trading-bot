...
- `docs/HERMES_CRON_JOB_DESIGN.md` defines the first future Hermes cron job as status-only: repo safety, Hermes cron readiness, VPS monitoring status, and optional market-monitor scheduling readiness. It excludes `--refresh-promoted-review` and `--refresh-defensive-research` from the first job until a later separate review. Verify this checklist with `python scripts\verify_hermes_cron_job_design.py`.
