...
- `docs/HERMES_PAUSED_STATUS_CRON_CHECKPOINT.md` records the paused Hermes status job definition `paused-vps-safe-paper-bot-status-check` with job ID `66c8a5bb438e`. It is `enabled=false`, `state=paused`, has placeholder schedule `2099-01-01 00:00`, has never run, and is not scheduling approval. Its intended sequence is repo safety, Hermes cron readiness, `verify_vps_daily_monitoring_summary.py`, and `--vps-daily-monitoring-summary`; standalone active-seed/paper-live report commands are intentionally omitted because the daily summary includes active-seed readiness. Verify with `python scripts\verify_hermes_paused_status_cron_checkpoint.py`.
