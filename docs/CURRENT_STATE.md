...
- The scheduling readiness report statically audits the refresh command, command inventory, ignored generated outputs, and safety boundaries. It does not create Windows Task Scheduler tasks, add cron/loop execution, approve scheduling, or approve execution.
- Future Hermes cron is preferred for monitoring-only, chat-delivered market monitor refresh reports once Hermes runs on the VPS. The candidate future command is `cd /d C:\dev\paper-trading-bot` followed by `.venv\Scripts\python.exe bot.py --refresh-market-monitor`; use no-agent mode for deterministic commands where possible.
- Windows Task Scheduler may still be used only to start or keep the Hermes gateway running on boot, not for execution-capable trading commands. Scheduling is not approved yet, and market monitor scheduling does not approve orders or paper execution.
