...
- Diagnostics now compare the original growth-biased strategy directly with `growth_biased_rotation_cost_aware_rebalance` and label whether any turnover reduction actually improves cost/split sensitivity without excessive CAGR, Sharpe, or Calmar drag. Reduced turnover alone is not treated as an improvement.
