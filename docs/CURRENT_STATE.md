...
- `docs/HERMES_PAUSED_STATUS_CRON_CHECKPOINT.md` records the paused Hermes status job definition `paused-vps-safe-paper-bot-status-check` with job ID `66c8a5bb438e`. It is `enabled=false`, `state=paused`, has placeholder schedule `2099-01-01 00:00`, has never run as a scheduled cron job, and is not scheduling approval. Its intended sequence is repo safety, Hermes cron readiness, `verify_vps_daily_monitoring_summary.py`, and `--vps-daily-monitoring-summary`; standalone active-seed/paper-live report commands are intentionally omitted because the daily summary includes active-seed readiness. The `2026-06-27` manual one-off test passed after missing saved active-seed readiness evidence was regenerated manually, and the job remained paused/disabled. Verify with `python scripts\verify_hermes_paused_status_cron_checkpoint.py`.
- `docs/HERMES_STATUS_CRON_ENABLEMENT_CHECKLIST.md` is the separate market-hours enablement planning checkpoint for that paused job. It documents the user's preference for at least hourly status checks while US markets are open, with 30-minute checks only after stable hourly runs, but it does not enable the job, approve a cadence, approve scheduling, call Alpaca, read positions, refresh market data, or add any order-capable command. Verify with `python scripts\verify_hermes_status_cron_enablement_checklist.py`.
