...
- Diagnostics now compare the original growth-biased strategy directly with `growth_biased_rotation_cost_aware_rebalance` and `growth_biased_rotation_partial_defensive_sleeve`. Reduced turnover alone is not treated as an improvement, and the partial defensive sleeve should only displace the original lead if it improves risk-adjusted performance or split stability without excessive CAGR, Sharpe, or Calmar drag. When those tested refinements show return drag without cost/split improvement, diagnostics stop recommending them as next work and move on to re-entry, split-stability, regime-recovery, and breadth-threshold review hypotheses.
