...
## Defensive Allocation Preview Status

- `--defensive-allocation-preview` reads saved `data/defensive_research_state_report.csv`.
- Its command orchestration lives in `trading_bot/runners/research_reports.py`; the preview logic lives in `trading_bot/research/defensive_allocation_preview.py`.
- It writes `data/defensive_allocation_preview.csv`.
- It summarizes lead defensive reference, secondary/split-sensitive checks, breadth diagnostic context, adaptive monitoring, paused short research, and the blocked execution gate.
- Every row is marked `research_only=True`, `preview_only=True`, and `execution_approved=False`.
...
## Defensive Allocation Decision Report Status

- `--defensive-allocation-decision-report` reads saved `data/defensive_allocation_preview.csv` and `data/defensive_allocation_risk_preview.csv`.
- Its command orchestration lives in `trading_bot/runners/research_reports.py`; the decision logic lives in `trading_bot/research/defensive_allocation_decision.py`.
- It writes `data/defensive_allocation_decision_report.csv`.
- It summarizes overall readiness, lead reference, preview safety, warning context, blocker context, and the next kill-switch/execution-readiness gate.
- Every row is marked `research_only=True`, `preview_only=True`, and `execution_approved=False`.
- It does not rerun backtests, download market data, call Alpaca, send Discord alerts, write to SQLite, create order instructions, promote strategies, approve execution, or move anything toward execution without future gates.
...
- `--etf-breadth-regime-robustness` reads saved breadth price history and writes fixed-split OOS rows for `split_60_40`, `split_70_30`, and `split_80_20`.
...
- It writes `data/etf_breadth_regime_backtest.csv`, `data/etf_breadth_regime_summary.csv`, `data/etf_breadth_regime_decision_report.csv`, and `data/etf_breadth_regime_robustness_report.csv`.
...
- The robustness report labels breadth regime conservatively and does not automatically promote it.
- Every row is marked `research_only=True`, `preview_only=True`, and `execution_approved=False`.
...
- It answers what still blocks future paper execution design for the defensive allocation path and identifies the next gate as real paper kill-switch enforcement design/tests.
- Every row is marked `research_only=True`, `preview_only=True`, and `execution_approved=False`.
