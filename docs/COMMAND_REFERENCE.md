...
This is the original long-form project README, preserved as a detailed command and behavior reference. For the GitHub landing page, see [README.md](../README.md).
...
QQQ100 postcheck readiness mode is a saved-output/runbook-only checkpoint for the VPS postcheck evidence state. If `data\qqq100_paper_postcheck.csv` or `position_quantity_abs_or_current_position_quantity_abs` is missing, it names the exact missing evidence. If saved postcheck evidence is present, it reports the saved postcheck status, recent-order match flag, saved QQQ position quantity, and alignment state. It explains that the only relevant future evidence command is the existing read-only postcheck, `python bot.py --qqq100-paper-postcheck --confirm-readonly-alpaca-check`, and that this command must not be run or rerun without explicit user approval. The postcheck may read broker order/position state after confirmation, but it must never create, submit, cancel, replace, or prepare orders, and its output still does not approve follow-up/repeat paper orders.

Command:

```text
python bot.py --qqq100-postcheck-readiness-report
```

Saved display:

```text
python bot.py --show-qqq100-postcheck-readiness-report
```
...
This runbook does not call Alpaca, read live positions, run postcheck, run QQQ100 paper execution, or approve execution. Every row preserves `execution_approved=false`, `paper_execution_approved=false`, `scheduling_approved=false`, `live_trading_approved=false`, and `followup_order_approved=false`.

QQQ100 follow-up policy mode is the saved-output/report-only no-action policy checkpoint for the current QQQ100 paper state. It reads saved paper-live evidence only. If desired state is `long` and saved QQQ position is long exactly one share, it reports `no_action_required_already_aligned`, does not allow another buy, and keeps repeat/follow-up execution false. If desired state is `flat` while saved QQQ is long exactly one share, it labels only `future_manual_flatten_discussion_possible`; if desired state is `long` while saved QQQ is flat, it labels only `future_manual_buy_discussion_possible`. Missing, fractional, excess, or contradictory saved quantity evidence blocks/manual-review.

Command:

```text
python bot.py --qqq100-followup-policy-report
```

Saved display:

```text
python bot.py --show-qqq100-followup-policy-report
```
...
QQQ100 daily decision mode is the saved-output/report-only checkpoint for turning the current QQQ100 evidence into a plain daily status. It reads saved QQQ100 paper-live evidence and the saved follow-up/no-action policy only. It can say hold/no-action aligned long, hold/no-action aligned flat, manual buy discussion possible but not approved, manual flatten discussion possible but not approved, or blocked/manual-review. It does not call Alpaca, read live positions, refresh market data, create executable order instructions, run QQQ100 paper execution, approve execution, approve repeat/follow-up orders, approve scheduling, or approve live trading.

Command:

```text
python bot.py --qqq100-daily-decision-report
```

Saved display:

```text
python bot.py --show-qqq100-daily-decision-report
```
...
Expected current status when QQQ100 is already aligned long one share is `qqq100_daily_decision_hold_no_action_aligned_long` with `recommended_next_step=hold_no_action_and_monitor_only`.

QQQ100 manual flatten readiness mode is the saved-output/report-only checkpoint for a future case where QQQ100's saved desired state becomes flat while saved QQQ evidence still shows long exactly one share. It reads saved QQQ100 paper-live evidence and the saved follow-up/no-action policy only. It can say `flatten_not_needed_currently`, `flatten_not_needed_already_flat`, `future_manual_flatten_discussion_possible_not_approved`, or `blocked_manual_review_required`. It does not call Alpaca, read live positions, refresh market data, create executable order instructions, run QQQ100 paper execution, approve flatten execution, approve repeat/follow-up orders, approve scheduling, or approve live trading.

Command:

```text
python bot.py --qqq100-manual-flatten-readiness-report
```

Saved display:

```text
python bot.py --show-qqq100-manual-flatten-readiness-report
```
...
```text
python bot.py --paper-live-monitoring-status
```

Saved display:

```text
python bot.py --show-paper-live-monitoring-status
```
...
If saved evidence is missing, the status reports `missing_saved_evidence` or manual review, without failing the monitoring display. Every row preserves `execution_approved=false`, `paper_execution_approved=false`, `scheduling_approved=false`, `live_trading_approved=false`, `followup_order_approved=false`, `repeat_execution_approved=false`, and `never_schedule_order_capable_commands=true`.

Paper-live checklist status mode is the saved-output/report-only closeout checkpoint for the current QQQ100 paper-live monitoring phase. It reads `data/paper_live_monitoring_status.csv`, classifies PAPER_LIVE_CHECKLIST Steps 1-11 as complete or complete-for-current-QQQ100-monitoring-phase, keeps Step 12 as future-only, and explains why no further QQQ order is needed while QQQ100 is aligned long one share. It does not approve live-money trading, paper execution, repeat/follow-up orders, scheduling, or a generic promotion ladder.

Command:

```text
python bot.py --paper-live-checklist-status
```

Saved display:

```text
python bot.py --show-paper-live-checklist-status
```
...
Paper-live promotion ladder design mode is the saved-output/report-only checkpoint for a future generic ladder. It documents the stage labels `research_candidate`, `preview_candidate`, `paper_live_candidate`, and `manually_executable_candidate`, but does not implement real promotion, execution, scheduling, or order instructions. The current report/status seed is `higher_growth_multi_sleeve_target_vol_15_win_20_cap_1x` / `MULTI_SLEEVE`, while QQQ100 remains previous-seed context, monitor-only and aligned long one share. No repeat/follow-up/flatten QQQ order is approved, and the current saved flatten labels are `flatten_not_needed_currently` and `manual_flatten_runbook_not_needed_currently`. High-growth and crypto remain research-only, defensive sleeves remain future review only, and no SMA or slow-SMA paper-live promotion is allowed. Portfolio backtests are not promotion evidence until accounting consistency is proven. Unknown position state must block/manual-review rather than assume flat. No scheduled execution is allowed.
...
Paper-live high-growth evidence-gap audit mode is the saved-output/report-only checkpoint for the future high-growth sleeve before any QQQ-led multi-sleeve promotion-ladder discussion. It checks saved-output file presence only across high-growth lead evidence, concentration/top-contributor dependency evidence, drawdown-window evidence, component attribution, survivorship/current-constituent/outlier warnings, and promotion-readiness blockers. Missing saved outputs are blockers/manual-review items, not failures and not execution approval. No high-growth sleeve is promoted, no action previews or order instructions are created, no research is rerun, no market data is refreshed, no portfolio execution is wired, and no scheduling is approved.

Command:

```text
python bot.py --paper-live-high-growth-evidence-gap
```

Saved display:

```text
python bot.py --show-paper-live-high-growth-evidence-gap
```
...
Paper-live high-growth manual-review decision mode is the saved-output checkpoint after the high-growth evidence-gap and evidence-quality reviews. It reads only those saved outputs, summarizes the current manual-review reason, and keeps the high-growth branch research-only when the saved quality review shows outlier dependence, one-name concentration, severe drawdown, survivorship/current-constituent warnings, or `high_growth_stock_outlier_dependent`. QQQ100 remains the cleaner current paper-live monitor base. High-growth is not permanently rejected, but future reconsideration requires concentration control evidence, component/drawdown attribution review with acceptable dependency, split/cost review, portfolio accounting consistency, F6/F7 compatibility, risk policy review, and no order instructions or scheduling.

Command:

```text
python bot.py --paper-live-high-growth-manual-review-decision
```

Saved display:

```text
python bot.py --show-paper-live-high-growth-manual-review-decision
```
...
Expected decision status is `high_growth_remains_research_only_manual_review_required`. No high-growth preview candidate, paper-live candidate, promotion, action preview, order instruction, execution wiring, market refresh, broker call, or scheduling is approved.
