...
Project research state refresh is the consolidated stock/ETF plus crypto checkpoint before choosing the next research/reporting direction. `python bot.py --project-research-state-refresh` reads saved Codex ambitious stock/ETF lead-decision, validation, split/drawdown, stricter-gate context, crypto universe, crypto robustness, crypto risk-control, crypto lead-decision, split-sensitivity, and crypto manual-review outputs where available, then writes `data/project_research_state_refresh.csv`, `data/project_research_state_summary.csv`, and `data/project_research_state_next_steps.csv`. It consolidates current stock/ETF and crypto research state, blockers, rejected or downgraded branches, safety boundaries, and suggested next options. This is research/report-only, does not approve preview promotion, does not approve execution, does not connect strategies to Alpaca or paper orders, and exists to choose the next research/reporting direction rather than create strategy-to-execution wiring.

Current research state display is the concise terminal display helper for the saved project checkpoint. `python bot.py --show-current-research-state` reads saved project research state, does not refresh market data, does not approve preview promotion, does not approve execution, and does not connect strategies to Alpaca or paper orders.
