...
Strategy improvement lab is the next ETF research-only checkpoint for reducing excessive cash drag without approving execution. `python bot.py --strategy-improvement-lab` tests a fixed small set of monthly ETF allocation variants: the monthly ETF rotation reference, balanced dual momentum with defensive sleeve, breadth-aware risk-on rotation, growth-biased rotation with crash gate, cost-aware growth-biased rebalance refinement, partial defensive-sleeve growth-biased refinement, factor/style absolute-gate rotation, sector 52-week-high continuation, and an ambitious fixed multi-sleeve growth allocator. It uses daily yfinance ETF history, fixed 126-day momentum, fixed 200-day trend checks, fixed 52-week-high closeness scoring where relevant, fixed 60/40/30 breadth thresholds, fixed 20/252-day volatility diagnostics, a fixed 5 percentage-point cost-aware rebalance threshold, fixed 75/25 mixed and 50/50 weak partial defensive-sleeve allocations, SPY and equal-weight benchmarks, and chronological in/out-of-sample reporting. `python bot.py --show-strategy-improvement-lab` reads the saved summary CSV only. `python bot.py --strategy-improvement-robustness` compares all candidates across fixed 60/40, 70/30, and 80/20 chronological splits, fixed low/default/high cost assumptions, drawdown windows, cash drag, and benchmark deltas. `python bot.py --strategy-improvement-diagnostics` then reads the saved lab/robustness CSVs only to explain why the current active lead, `growth_biased_rotation_crash_gate`, is split-sensitive and whether `growth_biased_rotation_cost_aware_rebalance` or `growth_biased_rotation_partial_defensive_sleeve` improves the diagnosed weakness without excessive return drag. If those refinements fail, diagnostics keep them as tested/rejected lab variants and recommend future fixed hypotheses such as re-entry filtering, split-stability checking, regime-recovery filtering, and breadth-threshold review. Promising labels are research prompts only; they do not approve buy/sell signals, order instructions, paper execution, scheduling, shorting, leverage, margin, or strategy-to-execution wiring.
