...
- **VPS handoff:** Manual update flow is `git pull`, `.venv\Scripts\python.exe scripts\verify_repo_safety.py`, then `.venv\Scripts\python.exe scripts\verify_monitor_lockfile_final_state.py`. Safe manual monitoring commands are `.venv\Scripts\python.exe bot.py --monitor-lockfile-readiness-report`, `.venv\Scripts\python.exe bot.py --refresh-promoted-review`, and `.venv\Scripts\python.exe bot.py --refresh-defensive-research`; these are not scheduling approval or execution approval.
