...
- **VPS handoff:** Manual update flow is `git pull`, `py -3 scripts\verify_repo_safety.py`, then `py -3 scripts\verify_monitor_lockfile_final_state.py`. Safe manual monitoring commands are `py -3 bot.py --monitor-lockfile-readiness-report`, `py -3 bot.py --refresh-promoted-review`, and `py -3 bot.py --refresh-defensive-research`; these are not scheduling approval or execution approval.
