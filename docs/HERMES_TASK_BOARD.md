...
- **Growth-biased stricter validation checkpoint:** `python scripts\verify_growth_biased_stricter_validation.py` verifies `--growth-biased-stricter-validation` and `--show-growth-biased-stricter-validation` remain saved-output validation/display only, write ignored validation CSVs, keep `growth_biased_rotation_breadth_stricter_gate` as research lead, and preserve false execution approval flags.
