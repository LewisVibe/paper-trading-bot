...
- **Scheduling state:** No refresh cron job is approved. The existing status job remains read-only. The sole execution-scheduling exception is `--run-vol-targeted-growth-auto-paper` under `docs/HERMES_AUTO_PAPER_EXECUTION_CRON.md`; every manual or live route remains forbidden.
...
- **Never schedule:** Normal `python bot.py`, paper-order tests, slow-SMA paper execution, or any execution-capable command except the explicitly approved `--run-vol-targeted-growth-auto-paper` route. No route becomes schedulable merely because a lockfile exists.
...
- **Daily summary command:** `python bot.py --vps-daily-monitoring-summary` is report/display-only for concise Telegram/manual checks. It summarizes promoted/defensive/freshness state plus saved QQQ100 paper-live monitoring status and the saved volatility-targeted candidate decision record when available, adds `action_required`, `action_reason`, and `suggested_manual_action`, and keeps `execution_approved=False` and `scheduling_approved=False`.
- **Execution-design approval visibility:** the daily summary and go/no-go dashboard may show `vol_execution_design_approved=True` once `python bot.py --vol-targeted-growth-execution-design-approval-record` has been run, but that is design-only approval. It must still keep order values blank, no executable ticket created, no orders submitted, and execution/paper-execution/scheduling approval false.
- **Non-submitting executable-ticket design visibility:** `python bot.py --vol-targeted-growth-non-submitting-executable-ticket-design` may create a saved design artifact after design-only approval. Hermes summaries may show the decision, but this remains monitor/report-only: order values stay blank, no executable ticket exists, no order is submitted, and scheduling/execution approvals remain false.
...
### Task: Volatility-targeted growth broker-comparison run-readiness checkpoint
- **Purpose:** Use `python bot.py --vol-targeted-growth-broker-comparison-run-readiness` to check whether the saved chain is ready to request explicit manual approval for a future read-only broker-position comparison.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-broker-comparison-run-readiness`, `python bot.py --show-vol-targeted-growth-broker-comparison-run-readiness`, and `python scripts\verify_vol_targeted_growth_broker_comparison_run_readiness.py`.
...
### Task: Volatility-targeted growth post-comparison decision
- **Purpose:** Use `python bot.py --vol-targeted-growth-post-comparison-decision` to interpret saved broker-position comparison output and decide whether the chain is ready to design a stricter manual paper-live discussion gate.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-post-comparison-decision`, `python bot.py --show-vol-targeted-growth-post-comparison-decision`, and `python scripts\verify_vol_targeted_growth_post_comparison_decision.py`.
- **Expected status:** `vol_targeted_growth_stricter_paper_live_discussion_gate_ready_manual_review_required`; the stricter gate is not approved or implemented.
...
- **Stop condition:** Stop if the task would call Alpaca, read positions, approve the stricter gate, approve paper-live candidacy, create order fields, or connect the candidate to execution.

### Task: Volatility-targeted growth stricter paper-live gate design
- **Purpose:** Use `python bot.py --vol-targeted-growth-stricter-paper-live-gate-design` to define hard blockers before any paper-live discussion of the volatility-targeted 15/20 candidate.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-stricter-paper-live-gate-design`, `python bot.py --show-vol-targeted-growth-stricter-paper-live-gate-design`, and `python scripts\verify_vol_targeted_growth_stricter_paper_live_gate_design.py`.
...
### Task: Volatility-targeted growth candidate discussion
- **Purpose:** Use `python bot.py --vol-targeted-growth-candidate-discussion` to compare QQQ100 and the volatility-targeted candidate in plain terms and decide whether the latter can be a non-executable proposal for manual review.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-candidate-discussion`, `python bot.py --show-vol-targeted-growth-candidate-discussion`, and `python scripts\verify_vol_targeted_growth_candidate_discussion.py`.
...
### Task: Volatility-targeted growth proposal preview
- **Purpose:** Use `python bot.py --vol-targeted-growth-proposal-preview` to create saved sleeve-level proposal rows from the approved schema for manual review only.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-proposal-preview`, `python bot.py --show-vol-targeted-growth-proposal-preview`, and `python scripts\verify_vol_targeted_growth_proposal_preview.py`.
...
### Task: Volatility-targeted growth seed-change evidence pack
- **Purpose:** Use `python bot.py --vol-targeted-growth-seed-change-evidence-pack` to list the evidence required before QQQ100 displacement could even be proposed.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-seed-change-evidence-pack`, `python bot.py --show-vol-targeted-growth-seed-change-evidence-pack`, and `python scripts\verify_vol_targeted_growth_seed_change_evidence_pack.py`.
- **Expected status:** `vol_targeted_growth_seed_change_evidence_pack_incomplete_manual_review_required`; QQQ100 remains the seed and displacement is not approved.
- **Forbidden commands:** Broker reads, seed changes, seed-change proposal creation, order creation, executable/account/secret fields, market refresh, action preview implementation, portfolio execution wiring, high-growth/crypto promotion, scheduling, or paper-live approval.
- **Stop condition:** Stop if the task would create a seed-change proposal, change the seed, approve QQQ100 displacement, read positions, create order fields, call Alpaca, or connect the candidate to execution.

### Task: Volatility-targeted growth seed-change risk/reward comparison
- **Purpose:** Use `python bot.py --vol-targeted-growth-seed-change-risk-reward-comparison` to compare saved QQQ100 and volatility candidate metrics as one evidence item.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-seed-change-risk-reward-comparison`, `python bot.py --show-vol-targeted-growth-seed-change-risk-reward-comparison`, and `python scripts\verify_vol_targeted_growth_seed_change_risk_reward_comparison.py`.
- **Expected status:** `vol_targeted_growth_risk_reward_evidence_created_manual_review_required`; QQQ100 remains the seed and displacement is not approved.
- **Forbidden commands:** Broker reads, market refresh, seed changes, seed-change proposal creation, order creation, executable/account/secret fields, action preview implementation, portfolio execution wiring, scheduling, or paper-live approval.
...
### Task: Volatility-targeted growth seed-change drawdown/stress review
- **Purpose:** Use `python bot.py --vol-targeted-growth-seed-change-drawdown-stress-review` to compare saved QQQ100 and volatility candidate MaxDD evidence as one evidence item.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-seed-change-drawdown-stress-review`, `python bot.py --show-vol-targeted-growth-seed-change-drawdown-stress-review`, and `python scripts\verify_vol_targeted_growth_seed_change_drawdown_stress_review.py`.
- **Expected status:** `vol_targeted_growth_drawdown_stress_evidence_created_manual_review_required`; QQQ100 remains the seed and displacement is not approved.
- **Forbidden commands:** Broker reads, market refresh, fresh stress regeneration, seed changes, seed-change proposal creation, order creation, executable/account/secret fields, action preview implementation, portfolio execution wiring, scheduling, or paper-live approval.
- **Stop condition:** Stop if the task would refresh metrics, create a seed-change proposal, change the seed, approve QQQ100 displacement, read positions, create order fields, call Alpaca, or connect the candidate to execution.

### Task: Volatility-targeted growth seed-change cost/turnover review
- **Purpose:** Use `python bot.py --vol-targeted-growth-seed-change-cost-turnover-review` to record the cost/turnover evidence state for the volatility seed-change checklist.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-seed-change-cost-turnover-review`, `python bot.py --show-vol-targeted-growth-seed-change-cost-turnover-review`, and `python scripts\verify_vol_targeted_growth_seed_change_cost_split_reviews.py`.
- **Expected status:** `vol_targeted_growth_cost_turnover_evidence_created_manual_review_required`; exact cost/turnover stress remains missing and QQQ100 displacement is not approved.
- **Forbidden commands:** Broker reads, market refresh, fresh cost stress, seed changes, seed-change proposal creation, order creation, executable/account/secret fields, action preview implementation, portfolio execution wiring, scheduling, or paper-live approval.
- **Stop condition:** Stop if the task would refresh metrics, create a seed-change proposal, change the seed, approve QQQ100 displacement, read positions, create order fields, call Alpaca, or connect the candidate to execution.

### Task: Volatility-targeted growth seed-change split-stability review
- **Purpose:** Use `python bot.py --vol-targeted-growth-seed-change-split-stability-review` to record saved split-stability evidence for the volatility seed-change checklist.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-seed-change-split-stability-review`, `python bot.py --show-vol-targeted-growth-seed-change-split-stability-review`, and `python scripts\verify_vol_targeted_growth_seed_change_cost_split_reviews.py`.
...
### Task: Volatility-targeted growth seed-change broker-exposure review
- **Purpose:** Use `python bot.py --vol-targeted-growth-seed-change-broker-exposure-review` to review saved broker-comparison output as the final evidence-pack context item.
- **Risk level:** Low/report-only when limited to saved outputs and no fresh broker reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-seed-change-broker-exposure-review`, `python bot.py --show-vol-targeted-growth-seed-change-broker-exposure-review`, and `python scripts\verify_vol_targeted_growth_seed_change_remaining_evidence_reviews.py`.
...
### Task: Volatility-targeted growth formal seed-change proposal
- **Purpose:** Use `python bot.py --vol-targeted-growth-formal-seed-change-proposal` to create the saved human-review proposal document for a possible future seed change.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-formal-seed-change-proposal`, `python bot.py --show-vol-targeted-growth-formal-seed-change-proposal`, and `python scripts\verify_vol_targeted_growth_formal_seed_change_proposal.py`.
- **Expected status:** `vol_targeted_growth_formal_seed_change_proposal_created_manual_approval_required`; QQQ100 remains the seed and approval is not recorded.
- **Forbidden commands:** Manual approval recording, seed changes, broker reads, Alpaca calls, market refresh, order creation, executable/account/secret fields, action preview implementation, portfolio execution wiring, scheduling, or paper-live approval.
- **Stop condition:** Stop if the task would record approval, change the seed, approve QQQ100 displacement, read positions, create order fields, call Alpaca, or connect the candidate to execution.

### Task: Volatility-targeted growth seed-change manual approval record
- **Purpose:** Use `python bot.py --vol-targeted-growth-seed-change-manual-approval-record` to record approval for designing the seed-change implementation only.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-seed-change-manual-approval-record`, `python bot.py --show-vol-targeted-growth-seed-change-manual-approval-record`, and `python scripts\verify_vol_targeted_growth_seed_change_manual_approval_record.py`.
- **Expected status:** `vol_targeted_growth_seed_change_manual_approval_recorded_implementation_required`; QQQ100 remains the active seed until a separate implementation checkpoint.
...
### Task: Volatility-targeted growth seed-change implementation design
- **Purpose:** Use `python bot.py --vol-targeted-growth-seed-change-implementation-design` to describe the future code-change boundaries for a possible seed switch.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-seed-change-implementation-design`, `python bot.py --show-vol-targeted-growth-seed-change-implementation-design`, and `python scripts\verify_vol_targeted_growth_seed_change_implementation_design.py`.
...
### Task: Volatility-targeted growth active-seed readiness
- **Purpose:** Use `python bot.py --vol-targeted-growth-active-seed-readiness` to check saved monitoring/status consistency for the active volatility-targeted report/status seed.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
...
### Task: Volatility-targeted growth allocation-cap and sleeve-mapping policy
- **Purpose:** Use `python bot.py --vol-targeted-growth-allocation-cap-sleeve-mapping-policy` to document allocation-cap and sleeve-mapping boundaries before any non-executable target-position design.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-allocation-cap-sleeve-mapping-policy`, `python bot.py --show-vol-targeted-growth-allocation-cap-sleeve-mapping-policy`, and `python scripts\verify_vol_targeted_growth_paper_live_checkpoints.py`.
- **Expected status:** `vol_targeted_growth_allocation_cap_sleeve_mapping_policy_created_manual_review_required`; executable allocation remains zero until a separate execution design exists, QQQ is review-only, high-growth and crypto remain research-only, defensive remains unmapped, and target-position design is not approved.
- **Forbidden commands:** Broker reads, Alpaca calls, market refresh, order creation, executable target positions, executable/account/secret fields, portfolio execution wiring, scheduling, paper-live execution approval, or treating the policy as an order approval.
- **Stop condition:** Stop if the task would call Alpaca, read positions, refresh yfinance data, create order fields, approve target positions, approve execution, schedule anything, or connect the candidate to execution.

### Task: Volatility-targeted growth non-executable target-position plan
- **Purpose:** Use `python bot.py --vol-targeted-growth-non-executable-target-position-plan` to document review-only target context after the allocation-cap and sleeve-mapping policy.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-non-executable-target-position-plan`, `python bot.py --show-vol-targeted-growth-non-executable-target-position-plan`, and `python scripts\verify_vol_targeted_growth_paper_live_checkpoints.py`.
- **Expected status:** `vol_targeted_growth_non_executable_target_position_plan_created_manual_review_required`; QQQ is review-only with no order quantity, high-growth and crypto remain research-only, defensive remains unmapped, and executable target positions are not created.
- **Forbidden commands:** Broker reads, Alpaca calls, market refresh, order creation, executable target positions, order side/quantity/type fields, account/secret fields, portfolio execution wiring, scheduling, paper-live execution approval, or treating review context as an order approval.
- **Stop condition:** Stop if the task would call Alpaca, read positions, refresh yfinance data, create order fields, approve target positions, approve execution, schedule anything, or connect the candidate to execution.

### Task: Volatility-targeted growth order-ticket boundary design
- **Purpose:** Use `python bot.py --vol-targeted-growth-order-ticket-boundary-design` to document which order-ticket fields and actions remain forbidden before any future executable design.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-order-ticket-boundary-design`, `python bot.py --show-vol-targeted-growth-order-ticket-boundary-design`, and `python scripts\verify_vol_targeted_growth_paper_live_checkpoints.py`.
- **Expected status:** `vol_targeted_growth_order_ticket_boundary_design_created_manual_review_required`; QQQ is review-only with no side or quantity, high-growth and crypto remain research-only, defensive remains unmapped, and executable order tickets are not created.
...
### Task: Volatility-targeted growth manual execution-design approval gate
- **Purpose:** Use `python bot.py --vol-targeted-growth-manual-execution-design-approval-gate` to define the exact future approval wording/scope required before non-submitting executable ticket design could be discussed.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-manual-execution-design-approval-gate`, `python bot.py --show-vol-targeted-growth-manual-execution-design-approval-gate`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_manual_execution_design_approval_gate.py`.
- **Expected status:** `vol_targeted_growth_manual_execution_design_approval_gate_not_approved` with `final_approval_gate_decision=MANUAL_EXECUTION_DESIGN_APPROVAL_NOT_RECORDED`; no approval, order fields, executable ticket, execution approval, or scheduling approval exists.
- **Monitoring integration:** `python bot.py --vps-daily-monitoring-summary` surfaces the saved approval-gate status when `data/vol_targeted_growth_manual_execution_design_approval_gate_summary.csv` exists; if missing, it reports missing saved output only. No cron job is created or edited and no order-capable command is added.
- **Forbidden commands:** Broker reads, Alpaca calls, market refresh, order creation, executable ticket design, side/quantity/order-type/time-in-force fields, account/secret fields, portfolio execution wiring, scheduling changes, paper-live execution approval, or treating the gate as approval.
- **Stop condition:** Stop if the task would call Alpaca, read positions, refresh yfinance data, create ticket fields, record approval, approve execution, schedule anything, or connect the candidate to execution.

### Task: Volatility-targeted growth non-submitting ticket schema design
- **Purpose:** Use `python bot.py --vol-targeted-growth-non-submitting-ticket-schema-design` to define future ticket schema fields for manual review after explicit approval for design work.
- **Risk level:** Low/design-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-non-submitting-ticket-schema-design`, `python bot.py --show-vol-targeted-growth-non-submitting-ticket-schema-design`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_non_submitting_ticket_schema_design.py`.
- **Expected status:** `vol_targeted_growth_non_submitting_ticket_schema_design_created_manual_review_required` with `final_schema_design_decision=NON_SUBMITTING_TICKET_SCHEMA_DESIGNED_NO_TICKET_CREATED`; no ticket instance, populated order values, order instruction, execution approval, or scheduling approval exists.
- **Monitoring integration:** `python bot.py --vps-daily-monitoring-summary` surfaces the saved schema-design status when `data/vol_targeted_growth_non_submitting_ticket_schema_design_summary.csv` exists; if missing, it reports missing saved output only. No cron job is created or edited and no order-capable command is added.
- **Forbidden commands:** Broker reads, Alpaca calls, market refresh, order creation/submission/cancellation/replacement, populated side/quantity/order-type/time-in-force values, account/secret fields, portfolio execution wiring, scheduling changes, or treating schema design as order approval.
- **Stop condition:** Stop if the task would call Alpaca, read positions, refresh yfinance data, create an actual ticket instance, populate order values, approve execution, schedule anything, or connect the candidate to execution.

### Task: Volatility-targeted growth non-submitting ticket-instance design
- **Purpose:** Use `python bot.py --vol-targeted-growth-non-submitting-ticket-instance-design` to define a draft future ticket-instance shape for manual review only.
- **Risk level:** Low/design-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-non-submitting-ticket-instance-design`, `python bot.py --show-vol-targeted-growth-non-submitting-ticket-instance-design`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_non_submitting_ticket_instance_design.py`.
- **Expected status:** `vol_targeted_growth_non_submitting_ticket_instance_design_created_manual_review_required` with `final_ticket_instance_design_decision=NON_SUBMITTING_TICKET_INSTANCE_DESIGNED_NO_ORDER_VALUES`; no executable ticket, populated order values, order instruction, execution approval, or scheduling approval exists.
- **Monitoring integration:** `python bot.py --vps-daily-monitoring-summary` surfaces the saved ticket-instance design status when `data/vol_targeted_growth_non_submitting_ticket_instance_design_summary.csv` exists; if missing, it reports missing saved output only. No cron job is created or edited and no order-capable command is added.
...
### Task: Volatility-targeted growth fresh broker pre-ticket gate run-readiness
- **Purpose:** Use `python bot.py --vol-targeted-growth-fresh-broker-pre-ticket-gate-run-readiness` to check whether the saved design chain is ready to request separate explicit read-only Alpaca approval.
- **Risk level:** Low/readiness-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-fresh-broker-pre-ticket-gate-run-readiness`, `python bot.py --show-vol-targeted-growth-fresh-broker-pre-ticket-gate-run-readiness`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_fresh_broker_pre_ticket_gate_run_readiness.py`.
- **Expected ready status:** `vol_targeted_growth_fresh_broker_pre_ticket_gate_run_readiness_ready_to_request_manual_readonly_approval` with `final_pre_ticket_gate_run_readiness_decision=READY_TO_REQUEST_EXPLICIT_READONLY_ALPACA_APPROVAL`; this is permission to ask for a future explicit approval prompt only, not permission to call Alpaca.
- **Monitoring integration:** `python bot.py --vps-daily-monitoring-summary` surfaces the saved run-readiness status when `data/vol_targeted_growth_fresh_broker_pre_ticket_gate_run_readiness_summary.csv` exists; if missing, it reports missing saved output only. No cron job is created or edited and no order-capable command is added.
- **Forbidden commands:** Broker reads, Alpaca calls, market refresh, order creation/submission/cancellation/replacement, populated side/quantity/order-type/time-in-force/account/broker-order-id values, account/secret fields, portfolio execution wiring, scheduling changes, or treating run-readiness as broker-read approval.
- **Stop condition:** Stop if the task would call Alpaca, read positions, refresh yfinance data, create an executable ticket, populate order values, approve execution, schedule anything, or connect the candidate to execution.

### Task: Volatility-targeted growth fresh broker pre-ticket gate run
- **Purpose:** Use `python bot.py --vol-targeted-growth-fresh-broker-pre-ticket-gate-run --confirm-readonly-alpaca-check` only after explicit user approval to save a fresh read-only Alpaca paper-position context before any future ticket-value discussion.
- **Risk level:** Medium/read-only broker check. It may call Alpaca and read paper positions, but it must never create tickets, populate order values, submit/cancel/replace orders, write trade logs, send alerts, approve execution, or approve scheduling.
- **Allowed commands:** `python bot.py --vol-targeted-growth-fresh-broker-pre-ticket-gate-run --confirm-readonly-alpaca-check`, `python bot.py --show-vol-targeted-growth-fresh-broker-pre-ticket-gate-run`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_fresh_broker_pre_ticket_gate_run.py`.
- **Expected successful status:** `vol_targeted_growth_fresh_broker_pre_ticket_gate_run_completed_readonly_manual_review_required`; the largest blocker should remain `ticket_values_not_approved_after_readonly_context`.
- **Monitoring integration:** `python bot.py --vps-daily-monitoring-summary` surfaces the saved read-only gate-run status when `data/vol_targeted_growth_fresh_broker_pre_ticket_gate_run_summary.csv` exists; if missing, it reports missing saved output only.
...
### Task: Volatility-targeted growth manual ticket-value design
- **Purpose:** Use `python bot.py --vol-targeted-growth-manual-ticket-value-design` to document which future ticket values would need separate explicit manual decisions.
- **Risk level:** Low/design-only when limited to saved outputs and blank executable fields.
- **Allowed commands:** `python bot.py --vol-targeted-growth-manual-ticket-value-design`, `python bot.py --show-vol-targeted-growth-manual-ticket-value-design`, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_manual_ticket_value_design.py`.
- **Expected status:** `vol_targeted_growth_manual_ticket_value_design_manual_review_required` with `final_ticket_value_design_decision=TICKET_VALUE_DESIGN_REVIEW_ONLY_VALUES_NOT_APPROVED`; `populated_ticket_value_count=0`.
- **Monitoring integration:** The paper-live go/no-go dashboard surfaces the saved ticket-value design status and still keeps `NO_GO_EXECUTION_BLOCKED_MONITOR_ONLY`.
...
### Task: Volatility-targeted growth executable-ticket approval criteria
- **Purpose:** Use `python bot.py --vol-targeted-growth-executable-ticket-approval-criteria` to define the manual criteria that would have to be reviewed before approval-readiness could ever become ready to ask.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-executable-ticket-approval-criteria`, `python bot.py --show-vol-targeted-growth-executable-ticket-approval-criteria`, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_executable_ticket_approval_criteria.py`.
- **Expected status:** `vol_targeted_growth_executable_ticket_approval_criteria_defined_manual_review_required` with `final_approval_criteria_decision=APPROVAL_CRITERIA_DEFINED_APPROVAL_NOT_REQUESTED`; approval is not requested or recorded.
- **Monitoring integration:** The paper-live go/no-go dashboard and VPS daily monitoring summary surface the saved approval-criteria decision when present.
- **Forbidden commands:** Broker reads, Alpaca calls, market refresh, approval requests, approval recording, order value population, order creation/submission/cancellation/replacement, executable ticket creation, strategy-to-execution wiring, cron edits, or treating criteria as approval.
- **Stop condition:** Stop if the task would request or record approval, populate executable order values, create an executable ticket, submit/cancel/replace orders, approve paper execution, approve live trading, or schedule anything.

### Task: Volatility-targeted growth executable-ticket criteria resolution plan
- **Purpose:** Use `python bot.py --vol-targeted-growth-executable-ticket-criteria-resolution-plan` to order the manual blocker-resolution work before any approval-readiness change.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-executable-ticket-criteria-resolution-plan`, `python bot.py --show-vol-targeted-growth-executable-ticket-criteria-resolution-plan`, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_executable_ticket_criteria_resolution_plan.py`.
- **Expected status:** `vol_targeted_growth_executable_ticket_criteria_resolution_plan_created_manual_review_required` with `final_resolution_plan_decision=CRITERIA_BLOCKER_RESOLUTION_PLAN_CREATED_APPROVAL_STILL_BLOCKED`; blockers are not resolved and approval remains blocked.
- **Monitoring integration:** The paper-live go/no-go dashboard and VPS daily monitoring summary surface the saved criteria-resolution decision when present.
- **Forbidden commands:** Broker reads, Alpaca calls, market refresh, blocker auto-resolution, approval requests, approval recording, order value population, order creation/submission/cancellation/replacement, executable ticket creation, strategy-to-execution wiring, cron edits, or treating the plan as approval.
- **Stop condition:** Stop if the task would resolve blockers automatically, request or record approval, populate executable order values, create an executable ticket, submit/cancel/replace orders, approve paper execution, approve live trading, or schedule anything.

### Task: Volatility-targeted growth executable-ticket criteria source review
- **Purpose:** Use `python bot.py --vol-targeted-growth-executable-ticket-criteria-source-review` to review saved criteria and resolution-plan wording/source before any blocker closeout discussion.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-executable-ticket-criteria-source-review`, `python bot.py --show-vol-targeted-growth-executable-ticket-criteria-source-review`, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_executable_ticket_criteria_source_review.py`.
- **Expected status:** `vol_targeted_growth_executable_ticket_criteria_source_review_manual_review_required` with `final_source_review_decision=CRITERIA_SOURCE_REVIEWED_NO_BLOCKERS_CLOSED`; source wording is reviewed but blockers remain open.
- **Monitoring integration:** The paper-live go/no-go dashboard and VPS daily monitoring summary surface the saved criteria source-review decision when present.
- **Forbidden commands:** Broker reads, Alpaca calls, market refresh, criteria edits, blocker auto-resolution, approval requests, approval recording, order value population, order creation/submission/cancellation/replacement, executable ticket creation, strategy-to-execution wiring, cron edits, or treating source review as approval.
- **Stop condition:** Stop if the task would change criteria, resolve blockers, request or record approval, populate executable order values, create an executable ticket, submit/cancel/replace orders, approve paper execution, approve live trading, or schedule anything.

### Task: Volatility-targeted growth executable-ticket criteria blocker closeout review
- **Purpose:** Use `python bot.py --vol-targeted-growth-executable-ticket-criteria-blocker-closeout-review` to classify criteria blockers for manual closeout review without closing them.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-executable-ticket-criteria-blocker-closeout-review`, `python bot.py --show-vol-targeted-growth-executable-ticket-criteria-blocker-closeout-review`, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_executable_ticket_criteria_blocker_closeout_review.py`.
- **Expected status:** `vol_targeted_growth_executable_ticket_criteria_blocker_closeout_review_manual_review_required` with `final_blocker_closeout_review_decision=CRITERIA_BLOCKERS_REVIEWED_NONE_CLOSED`; blockers remain open.
- **Monitoring integration:** The paper-live go/no-go dashboard and VPS daily monitoring summary surface the saved blocker closeout-review decision when present.
- **Forbidden commands:** Broker reads, Alpaca calls, market refresh, criteria edits, blocker closeout, approval-readiness changes, approval requests, approval recording, order value population, order creation/submission/cancellation/replacement, executable ticket creation, strategy-to-execution wiring, cron edits, or treating blocker review as approval.
- **Stop condition:** Stop if the task would change criteria, close blockers, change approval readiness, request or record approval, populate executable order values, create an executable ticket, submit/cancel/replace orders, approve paper execution, approve live trading, or schedule anything.

### Task: Volatility-targeted growth executable-ticket blocker-specific reviews
- **Purpose:** Use the first blocker-specific review commands to review `criteria_source_reviewed`, `criteria_resolution_plan_open`, and `approval_criteria_not_approval`, then roll them up without closing blockers.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-criteria-source-blocker-review`, `python bot.py --vol-targeted-growth-criteria-resolution-plan-blocker-review`, `python bot.py --vol-targeted-growth-approval-criteria-not-approval-blocker-review`, `python bot.py --vol-targeted-growth-criteria-blocker-specific-review-rollup`, their matching `--show-...` commands, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_executable_ticket_blocker_specific_reviews.py`.
- **Expected status:** The rollup records `vol_targeted_growth_criteria_blocker_specific_review_rollup_manual_review_required` with `final_review_decision=CRITERIA_BLOCKER_SPECIFIC_REVIEWS_CREATED_NONE_CLOSED`; blockers remain open.
- **Monitoring integration:** The paper-live go/no-go dashboard and VPS daily monitoring summary surface the saved blocker-specific review rollup decision when present.
- **Forbidden commands:** Broker reads, Alpaca calls, market refresh, criteria edits, blocker closeout, approval-readiness changes, approval requests, approval recording, order value population, order creation/submission/cancellation/replacement, executable ticket creation, strategy-to-execution wiring, cron edits, or treating blocker-specific review as approval.
- **Stop condition:** Stop if the task would change criteria, close blockers, change approval readiness, request or record approval, populate executable order values, create an executable ticket, submit/cancel/replace orders, approve paper execution, approve live trading, or schedule anything.

### Task: Volatility-targeted growth executable-ticket closeout-candidate reviews
- **Purpose:** Use closeout-candidate review commands to assess whether `criteria_source_reviewed`, `criteria_resolution_plan_open`, and `approval_criteria_not_approval` are ready for human closeout consideration without closing them.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-criteria-source-closeout-candidate-review`, `python bot.py --vol-targeted-growth-criteria-resolution-plan-closeout-candidate-review`, `python bot.py --vol-targeted-growth-approval-criteria-not-approval-closeout-candidate-review`, `python bot.py --vol-targeted-growth-criteria-closeout-candidate-review-rollup`, their matching `--show-...` commands, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_executable_ticket_closeout_candidate_reviews.py`.
- **Expected status:** The rollup records `vol_targeted_growth_criteria_closeout_candidate_review_rollup_manual_review_required` with `final_candidate_review_decision=CRITERIA_CLOSEOUT_CANDIDATES_REVIEWED_NONE_CLOSED`; one candidate may be ready for human review, but blockers remain open.
- **Monitoring integration:** The paper-live go/no-go dashboard and VPS daily monitoring summary surface the saved closeout-candidate review rollup decision when present.
- **Forbidden commands:** Broker reads, Alpaca calls, market refresh, criteria edits, blocker closeout, approval-readiness changes, approval requests, approval recording, order value population, order creation/submission/cancellation/replacement, executable ticket creation, strategy-to-execution wiring, cron edits, or treating candidate-ready status as approval.
- **Stop condition:** Stop if the task would close blockers, change approval readiness, request or record approval, populate executable order values, create an executable ticket, submit/cancel/replace orders, approve paper execution, approve live trading, or schedule anything.

### Task: Volatility-targeted growth criteria-source closeout approval wording
- **Purpose:** Use `python bot.py --vol-targeted-growth-criteria-source-closeout-approval-wording` to save the simple future phrase `I approve closing the criteria_source_reviewed blocker only.` without recording approval.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-criteria-source-closeout-approval-wording`, `python bot.py --show-vol-targeted-growth-criteria-source-closeout-approval-wording`, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_criteria_source_closeout_approval_wording.py`.
- **Expected status:** `vol_targeted_growth_criteria_source_closeout_approval_wording_manual_review_required` with `final_approval_wording_decision=CRITERIA_SOURCE_CLOSEOUT_APPROVAL_WORDING_DEFINED_NOT_APPROVED`; no blocker is closed and no approval is recorded.
- **Monitoring integration:** The paper-live go/no-go dashboard and VPS daily monitoring summary surface the saved wording decision and phrase when present.
...
### Task: Volatility-targeted growth criteria resolution-plan closeout
- **Purpose:** Use `python bot.py --vol-targeted-growth-criteria-resolution-plan-closeout-approval-wording` and `python bot.py --vol-targeted-growth-criteria-resolution-plan-closeout-record` to record closure of only `criteria_resolution_plan_open` after explicit user approval.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-criteria-resolution-plan-closeout-approval-wording`, `python bot.py --show-vol-targeted-growth-criteria-resolution-plan-closeout-approval-wording`, `python bot.py --vol-targeted-growth-criteria-resolution-plan-closeout-record`, `python bot.py --show-vol-targeted-growth-criteria-resolution-plan-closeout-record`, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_criteria_resolution_plan_closeout_record.py`.
- **Expected status:** `CRITERIA_RESOLUTION_PLAN_OPEN_BLOCKER_CLOSED_ONLY`; `closed_blocker_count=2` when combined with `criteria_source_reviewed`, while `approval_criteria_not_approval`, ticket values, executable-ticket prerequisites, execution, and scheduling remain blocked.
- **Forbidden commands:** Broker reads, Alpaca calls, market refresh, closing approval criteria, executable-ticket approval recording, order value population, order creation/submission/cancellation/replacement, executable ticket creation, strategy-to-execution wiring, cron edits, or treating this closeout as execution approval.
- **Stop condition:** Stop if the task would close additional blockers, populate executable order values, create an executable ticket, submit/cancel/replace orders, approve paper execution, approve live trading, or schedule anything.

### Task: Volatility-targeted growth approval-criteria closeout
- **Purpose:** Use `python bot.py --vol-targeted-growth-approval-criteria-closeout-approval-wording` and `python bot.py --vol-targeted-growth-approval-criteria-closeout-record` to record closure of only `approval_criteria_not_approval` after explicit user approval.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-approval-criteria-closeout-approval-wording`, `python bot.py --show-vol-targeted-growth-approval-criteria-closeout-approval-wording`, `python bot.py --vol-targeted-growth-approval-criteria-closeout-record`, `python bot.py --show-vol-targeted-growth-approval-criteria-closeout-record`, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_approval_criteria_closeout_record.py`.
- **Expected status:** `APPROVAL_CRITERIA_NOT_APPROVAL_BLOCKER_CLOSED_ONLY`; `closed_blocker_count=3` when combined with the previous closeouts, while ticket values, executable-ticket prerequisites, execution, and scheduling remain blocked.
- **Forbidden commands:** Broker reads, Alpaca calls, market refresh, executable-ticket approval recording, order value population, order creation/submission/cancellation/replacement, executable ticket creation, strategy-to-execution wiring, cron edits, or treating this closeout as execution approval.
- **Stop condition:** Stop if the task would populate executable order values, create an executable ticket, submit/cancel/replace orders, approve paper execution, approve live trading, or schedule anything.

### Task: Volatility-targeted growth final ticket blockers closeout
- **Purpose:** Use `python bot.py --vol-targeted-growth-final-ticket-blockers-closeout-approval-wording` and `python bot.py --vol-targeted-growth-final-ticket-blockers-closeout-record` to close `ticket_values_not_approved` and `executable_ticket_prerequisites_not_met` as checklist blockers only.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** `python bot.py --vol-targeted-growth-final-ticket-blockers-closeout-approval-wording`, `python bot.py --show-vol-targeted-growth-final-ticket-blockers-closeout-approval-wording`, `python bot.py --vol-targeted-growth-final-ticket-blockers-closeout-record`, `python bot.py --show-vol-targeted-growth-final-ticket-blockers-closeout-record`, `python bot.py --vol-targeted-growth-paper-live-execution-blocker-rollup`, `python bot.py --vol-targeted-growth-executable-ticket-gap-list`, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_final_ticket_blockers_closeout.py`.
- **Expected status:** `FINAL_TICKET_BLOCKERS_CLOSED_NO_EXECUTION_APPROVAL`; `closed_blocker_count=5`, `remaining_known_blockers_after_closeout=none`, and `largest_blocker=execution_not_approved`.
...
### Task: Volatility-targeted growth ticket-values discussion approval
- **Purpose:** Use `python bot.py --vol-targeted-growth-ticket-values-approval-readiness`, `python bot.py --vol-targeted-growth-ticket-values-approval-wording`, and `python bot.py --vol-targeted-growth-ticket-values-approval-record` to record permission to discuss future non-executable ticket-value placeholders only.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** The three report commands above, their matching `--show-...` commands, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_ticket_values_approval.py`.
- **Expected status:** `TICKET_VALUE_DISCUSSION_APPROVED_NO_ORDER_VALUES`; `ticket_value_discussion_approved=True` may be recorded, while `ticket_values_approved=False`, `order_values_populated=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals remain false.
...
### Task: Volatility-targeted growth proposed ticket values
- **Purpose:** Use `python bot.py --vol-targeted-growth-proposed-ticket-values` and `python bot.py --vol-targeted-growth-proposed-ticket-values-quality-gate` to draft review-only proposal labels and verify they are not broker-ready order fields.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** The two report commands above, their matching `--show-...` commands, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_proposed_ticket_values.py`.
- **Expected status:** `PROPOSED_TICKET_VALUES_QUALITY_GATE_PASSED_NO_EXECUTION`; `proposed_ticket_values_created=True` may be recorded, while `ticket_values_approved=False`, `order_values_populated=False`, `order_instructions_created=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals remain false.
- **Monitoring integration:** The paper-live go/no-go dashboard and VPS daily monitoring summary surface proposed-values and quality-gate decisions when present.
- **Stop condition:** Stop if the task would create broker-ready order fields, submit/cancel/replace orders, approve execution, or schedule anything.

### Task: Volatility-targeted growth executable-ticket draft readiness
- **Purpose:** Use `python bot.py --vol-targeted-growth-executable-ticket-draft-readiness` to check whether review-only proposed values are clear enough for a later manual non-submitting draft discussion.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** The report command above, its matching `--show-...` command, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_executable_ticket_draft_readiness.py`.
- **Expected status:** `READY_TO_DISCUSS_NON_SUBMITTING_DRAFT_VALUES_NOT_EXECUTABLE`; `draft_discussion_ready=True` may be recorded, while `ticket_values_approved=False`, `order_values_populated=False`, `order_instructions_created=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals remain false.
- **Monitoring integration:** The paper-live go/no-go dashboard and VPS daily monitoring summary surface draft-readiness decisions when present.
- **Stop condition:** Stop if the task would create a ticket, fill side, quantity, order type, time-in-force, price, account, broker fields, submit/cancel/replace orders, approve execution, or schedule anything.

### Task: Volatility-targeted growth non-submitting executable-ticket draft
- **Purpose:** Use `python bot.py --vol-targeted-growth-non-submitting-executable-ticket-draft` and `python bot.py --vol-targeted-growth-non-submitting-executable-ticket-draft-quality-gate` to create a review draft from proposed labels and verify it remains non-executable.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** The two report commands above, their matching `--show-...` commands, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_non_submitting_executable_ticket_draft.py`.
- **Expected status:** `NON_SUBMITTING_EXECUTABLE_TICKET_DRAFT_QUALITY_GATE_PASSED_NO_EXECUTION`; `draft_ticket_created=True` may be recorded, while `ticket_values_approved=False`, `order_values_populated=False`, `order_instructions_created=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals remain false.
- **Monitoring integration:** The paper-live go/no-go dashboard and VPS daily monitoring summary surface draft and draft-quality-gate decisions when present.
- **Stop condition:** Stop if the task would create broker-ready order fields, fill side, numeric quantity, executable order type, time-in-force, price, account, broker fields, submit/cancel/replace orders, approve execution, or schedule anything.

### Task: Volatility-targeted growth draft ticket-value approval readiness
- **Purpose:** Use `python bot.py --vol-targeted-growth-draft-ticket-value-approval-readiness` to check whether the non-submitting draft is complete enough to ask later for explicit ticket-value approval.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** The report command above, its matching `--show-...` command, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_draft_ticket_value_approval_readiness.py`.
- **Expected status:** `READY_TO_REQUEST_TICKET_VALUE_APPROVAL_NOT_APPROVED`; `ticket_value_approval_request_ready=True` may be recorded, while `ticket_value_approval_requested=False`, `ticket_value_approval_recorded=False`, `ticket_values_approved=False`, `order_values_populated=False`, `orders_submitted=False`, and all execution/scheduling approvals remain false.
- **Monitoring integration:** The paper-live go/no-go dashboard and VPS daily monitoring summary surface ticket-value approval-readiness decisions when present.
- **Stop condition:** Stop if the task would request or record approval, populate side, numeric quantity, executable order type, time-in-force, price, account, broker fields, submit/cancel/replace orders, approve execution, or schedule anything.

### Task: Volatility-targeted growth draft ticket-value approval wording and record
- **Purpose:** Use `python bot.py --vol-targeted-growth-draft-ticket-value-approval-wording` and `python bot.py --vol-targeted-growth-draft-ticket-value-approval-record` to define and record narrow approval for a later review-only draft-value population checkpoint.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** The wording and record commands above, their matching `--show-...` commands, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_draft_ticket_value_approval.py`.
- **Expected status:** `TICKET_VALUE_APPROVAL_WORDING_DEFINED_NOT_APPROVED` for wording and `TICKET_VALUE_APPROVAL_RECORDED_NO_ORDER_VALUES` for the record. The record may set `ticket_value_population_approved=True`, while `ticket_values_approved=False`, `order_values_populated=False`, `order_instructions_created=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals remain false.
...
### Task: Volatility-targeted growth non-submitting executable ticket values
- **Purpose:** Use `python bot.py --vol-targeted-growth-non-submitting-executable-ticket-values` and `python bot.py --vol-targeted-growth-non-submitting-executable-ticket-values-quality-gate` to populate and verify reviewable seed/sleeve context after the explicit approval record.
- **Risk level:** Low/report-only when limited to saved outputs and no broker or market-data reads.
- **Allowed commands:** The values and quality-gate commands above, their matching `--show-...` commands, `python bot.py --vol-targeted-growth-paper-live-execution-blocker-rollup`, `python bot.py --vol-targeted-growth-executable-ticket-gap-list`, `python bot.py --paper-live-go-no-go-dashboard`, `python bot.py --vps-daily-monitoring-summary`, and `python scripts\verify_vol_targeted_growth_non_submitting_executable_ticket_values.py`.
- **Expected status:** `NON_SUBMITTING_EXECUTABLE_TICKET_VALUES_POPULATED_REVIEW_ONLY` and `NON_SUBMITTING_EXECUTABLE_TICKET_VALUES_QUALITY_GATE_PASSED_NO_ORDER`. The pack may set `non_submitting_ticket_values_populated=True` and may carry saved review quantity estimates as manual-review context, while `broker_ready_order_values_populated=False`, `order_values_populated=False`, `order_instructions_created=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals remain false.
- **Monitoring integration:** The execution blocker rollup, executable ticket gap list, paper-live go/no-go dashboard, and VPS daily monitoring summary surface the non-submitting values and review quantity estimate state separately from broker-ready order values.
...
- **Expected status:** `NON_SUBMITTING_TICKET_INSTANCE_CHECKPOINT_CREATED_NO_ORDER_VALUES`. The checkpoint may set `ticket_instance_checkpoint_created=True` and may carry saved review quantity estimates as manual-review context, while `ticket_instance_created=False`, `ticket_creation_approved=False`, `broker_ready_order_values_populated=False`, `order_values_populated=False`, `order_instructions_created=False`, `orders_submitted=False`, and all execution/scheduling approvals remain false.
