...
- **Current scaffold commands:** `python bot.py --monitor-lockfile-readiness-report` and `python bot.py --refresh-promoted-review` are the only commands protected by the monitor lockfile helper. The locks are transient report-only no-overlap guards and do not approve scheduling or execution.
