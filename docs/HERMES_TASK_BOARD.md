...
- **First-job design:** `docs/HERMES_CRON_JOB_DESIGN.md` defines the first future Hermes cron job as status-only: repo safety, Hermes cron readiness, VPS monitoring status, and optional market-monitor scheduling readiness. It excludes refresh commands until a later review. Verify it with `python scripts\verify_hermes_cron_job_design.py`.
...
- **Hermes cron job design checkpoint:** `python scripts\verify_hermes_cron_job_design.py` verifies the first future cron job remains status-only, excludes refresh commands until later review, uses the VPS repo path and `.venv\Scripts\python.exe`, runs repo safety before status, avoids config/secrets/generated-content exposure, avoids recursive cron creation and Git writes, preserves false approval flags, and leaves the lock-wrapped set unchanged.
- **Current daily status cron:** `paper-bot-vps-status-check` is running as a status-only Hermes cron job. It does not approve scheduling changes or execution.
...
- **Promoted refresh cron design:** `docs/HERMES_PROMOTED_REVIEW_CRON_DESIGN.md` and `python scripts\verify_hermes_promoted_review_cron_design.py` document a possible future second Hermes cron job for lock-wrapped `--refresh-promoted-review` followed by status/summary output. No refresh cron job is currently created or approved.
