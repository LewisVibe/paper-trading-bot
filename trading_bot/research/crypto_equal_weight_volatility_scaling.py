"""Research-only equal-weight crypto volatility-scaling report.

This module tests fixed, auditable partial-exposure crypto risk controls around
the high-drawdown equal-weight crypto benchmark. It writes generated research
...
CODEX_RISK_CONTROL_STRATEGY = "codex_ambitious_crypto_core_alt_volatility_throttle"
SCALER_VARIANTS = [
...
ALL_NAMES = [*SCALER_VARIANTS, *BENCHMARKS]
...
OUTPUT_FILES = {
    "results": Path("data/crypto_equal_weight_volatility_scaling.csv"),
    "summary": Path("data/crypto_equal_weight_volatility_scaling_summary.csv"),
    "trades": Path("data/crypto_equal_weight_volatility_scaling_trades.csv"),
    "equity": Path("data/crypto_equal_weight_volatility_scaling_equity_curves.csv"),
    "costs": Path("data/crypto_equal_weight_volatility_scaling_costs.csv"),
    "splits": Path("data/crypto_equal_weight_volatility_scaling_splits.csv"),
    "drawdowns": Path("data/crypto_equal_weight_volatility_scaling_drawdowns.csv"),
}
...
@dataclass
class CryptoEqualWeightVolatilityScalingResult:
    results_path: Path
    summary_path: Path
    trades_path: Path
    equity_path: Path
...
def generate_crypto_equal_weight_volatility_scaling(data_dir: Path | str = "data") -> CryptoEqualWeightVolatilityScalingResult:
    ...
    return CryptoEqualWeightVolatilityScalingResult(
...
def show_crypto_equal_weight_volatility_scaling_file(data_dir: Path | str = "data") -> tuple[int, list[str]]:
    data_path = Path(data_dir)
    ...
    if not results or not summary:
        return 1, ["Run `python bot.py --crypto-equal-weight-volatility-scaling` first."]
    approvals = {str(row.get("execution_approved", "")).lower() for row in results + summary + costs + splits + drawdowns + equity}
    return 0, [
        "Crypto equal-weight volatility-scaling report. Display only; execution_approved=False.",
        f"Variants tested: {summary_value(summary, 'variants_tested')}",
        f"Codex-designed strategy: {summary_value(summary, 'codex_strategy_name')}",
        ...
        f"Best scaler variant: {summary_value(summary, 'best_scaler_variant')}",
        f"Best Codex risk-control variant: {summary_value(summary, 'best_codex_risk_control_variant')}",
        ...
        "Warning: volatility-scaling report does not create order instructions, approve preview promotion, or approve crypto execution.",
    ]
...
def run_split_checks(
    ...
    for split_name, split_pct in SPLITS:
        ...
        for name in ALL_NAMES:
            ...
            metric = add_decays(metric, metric)
            status = "split_credible" if float(metric.get("cagr_pct") or 0) > 0 and float(metric.get("calmar_ratio") or 0) > 0 else "crypto_vol_scaler_split_sensitive"
...
def simulate_name(
    ...
    *,
    ...
    if name == "crypto_equal_weight_volatility_scaled_allocator":
        return simulate_volatility_scaled_allocator(name, aligned, universe, cost_bps, created_at, include_trades)
    if name == "crypto_equal_weight_drawdown_scaled_allocator":
        return simulate_drawdown_scaled_allocator(name, aligned, universe, cost_bps, created_at, include_trades)
    if name == "crypto_equal_weight_combined_vol_drawdown_scaler":
        return simulate_combined_vol_drawdown_scaler(name, aligned, universe, cost_bps, created_at, include_trades)
    if name == CODEX_RISK_CONTROL_STRATEGY:
        return simulate_codex_core_alt_volatility_throttle(name, aligned, universe, cost_bps, created_at, include_trades)
    raise ValueError(f"Unsupported crypto equal-weight volatility-scaling variant: {name}")


def simulate_volatility_scaled_allocator(name: str, rows: list[dict[str, Any]], universe: list[str], cost_bps: int, created_at: str, include_trades: bool):
    def target(index: int, _weights: dict[str, float]) -> dict[str, float]:
        exposure = volatility_exposure(rows, universe, index)
        return scaled_equal_weights(universe, exposure)

    return simulate_weight_schedule(name, rows, cost_bps, created_at, include_trades, target, monthly_only=True)


def simulate_drawdown_scaled_allocator(name: str, rows: list[dict[str, Any]], universe: list[str], cost_bps: int, created_at: str, include_trades: bool):
    first_closes = rows[0]["closes"]
    peak = 1.0

    def target(index: int, _weights: dict[str, float]) -> dict[str, float]:
        nonlocal peak
        equal_weight_index = equal_weight_index_value(rows, universe, index, first_closes)
        peak = max(peak, equal_weight_index)
        exposure = drawdown_exposure((equal_weight_index / peak) - 1.0)
        return scaled_equal_weights(universe, exposure)

    return simulate_weight_schedule(name, rows, cost_bps, created_at, include_trades, target, monthly_only=True)


def simulate_combined_vol_drawdown_scaler(name: str, rows: list[dict[str, Any]], universe: list[str], cost_bps: int, created_at: str, include_trades: bool):
    first_closes = rows[0]["closes"]
    peak = 1.0
    ...
    def target(index: int, _weights: dict[str, float]) -> dict[str, float]:
        ...
        equal_weight_index = equal_weight_index_value(rows, universe, index, first_closes)
        peak = max(peak, equal_weight_index)
        exposure = max(0.25, min(volatility_exposure(rows, universe, index), drawdown_exposure((equal_weight_index / peak) - 1.0)))
        return scaled_equal_weights(universe, exposure)

    return simulate_weight_schedule(name, rows, cost_bps, created_at, include_trades, target, monthly_only=True)


def simulate_codex_core_alt_volatility_throttle(name: str, rows: list[dict[str, Any]], universe: list[str], cost_bps: int, created_at: str, include_trades: bool):
    ...
    def target(index: int, _weights: dict[str, float]) -> dict[str, float]:
        nonlocal peak
        if index < 252:
            return scaled_core_alt_weights(universe, ["BTC-USD", "ETH-USD"], 0.25)
        equal_weight_index = equal_weight_index_value(rows, universe, index, first_closes)
        peak = max(peak, equal_weight_index)
        breadth = sum(1 for symbol in universe if above_sma(rows, symbol, index, 200)) / max(1, len(universe))
        breadth_exposure = 1.0 if breadth >= 0.60 else 0.75 if breadth >= 0.40 else 0.50 if breadth >= 0.25 else 0.25
        exposure = max(0.25, min(volatility_exposure(rows, universe, index), drawdown_exposure((equal_weight_index / peak) - 1.0), breadth_exposure))
        ranked_alts = [symbol for symbol, _score in ranked_momentum(rows, alt_universe, index) if above_sma(rows, symbol, index, 200)]
        selected_alts = ranked_alts[:4]
        core = [symbol for symbol, _score in ranked_momentum(rows, ["BTC-USD", "ETH-USD"], index)]
        return scaled_core_alt_weights(universe, core, exposure, selected_alts)

    return simulate_weight_schedule(name, rows, cost_bps, created_at, include_trades, target, monthly_only=True)


def volatility_exposure(rows: list[dict[str, Any]], universe: list[str], index: int) -> float:
    if index < 315:
        return 1.0
    returns = equal_weight_returns(rows, universe, index - 252, index)
    recent = returns[-63:]
    history = [statistics.pstdev(returns[start : start + 63]) for start in range(0, max(0, len(returns) - 63))]
    median_vol = statistics.median([value for value in history if value > 0]) if history else 0.0
    recent_vol = statistics.pstdev(recent) if len(recent) >= 2 else 0.0
    if median_vol <= 0:
        return 1.0
    ratio = recent_vol / median_vol
    if ratio > 2.25:
        return 0.25
    if ratio > 1.75:
        return 0.50
...
def equal_weight_returns(rows: list[dict[str, Any]], universe: list[str], start: int, end: int) -> list[float]:
    values = []
    for index in range(max(1, start), min(end + 1, len(rows))):
        previous = rows[index - 1]["closes"]
        current = rows[index]["closes"]
        daily = sum((current[symbol] / previous[symbol]) - 1.0 for symbol in universe) / max(1, len(universe))
        values.append(daily)
    return values


def equal_weight_index_value(rows: list[dict[str, Any]], universe: list[str], index: int, first_closes: dict[str, float]) -> float:
    return sum(rows[index]["closes"][symbol] / first_closes[symbol] for symbol in universe) / max(1, len(universe))


def scaled_equal_weights(symbols: list[str], exposure: float) -> dict[str, float]:
    if not symbols:
        return {}
...
def scaled_core_alt_weights(universe: list[str], core_symbols: list[str], exposure: float, alt_symbols: list[str] | None = None) -> dict[str, float]:
    core = [symbol for symbol in core_symbols if symbol in universe][:2]
    if not core:
        core = [symbol for symbol in ["BTC-USD", "ETH-USD"] if symbol in universe]
    alts = [symbol for symbol in (alt_symbols or []) if symbol in universe and symbol not in core]
    weights: dict[str, float] = {}
    core_exposure = exposure * (0.60 if alts else 1.0)
    alt_exposure = exposure - core_exposure
    for symbol in core:
        weights[symbol] = core_exposure / len(core)
    for symbol in alts:
        weights[symbol] = alt_exposure / len(alts)
    return weights
...
def build_drawdown_rows(created_at: str, result_rows: list[dict[str, Any]], equity_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for name in ALL_NAMES:
        ...
        static_reference = static_dd if static_dd is not None else static_full_dd
        ...
        return_drag = cagr - static_cagr
        calmar_improvement = calmar - static_calmar
        evidence = (
            ...
            f"return_drag_vs_equal_weight={round(return_drag, 4)}; "
            f"calmar_improvement_vs_equal_weight={round(calmar_improvement, 4)}; "
        ...
        metric_value = (
            ...
            f"recovery_rows={window_text(window, 'recovery_rows')}; return_drag={round(return_drag, 4)}; "
            f"calmar_improvement={round(calmar_improvement, 4)}"
        )
        rows.append(
            common_row(
                created_at,
                ...
                label_drawdown(float(dd or 0), static_reference, return_drag, calmar_improvement),
                evidence,
            )
        )
    return rows
...
def build_summary_rows(
    ...
    scaler = best_scaler(full)
    codex = metric_for(full, CODEX_RISK_CONTROL_STRATEGY)
    static_cagr = extract_metric(static.get("metric_value", ""), "CAGR")
    static_dd = extract_metric(static.get("metric_value", ""), "MaxDD")
    static_calmar = extract_metric(static.get("metric_value", ""), "Calmar")
    scaler_cagr = extract_metric(scaler.get("metric_value", ""), "CAGR")
    scaler_dd = extract_metric(scaler.get("metric_value", ""), "MaxDD")
    scaler_calmar = extract_metric(scaler.get("metric_value", ""), "Calmar")
    dd_reduction = scaler_dd - static_dd
    return_drag = scaler_cagr - static_cagr
    calmar_improvement = scaler_calmar - static_calmar
    label = final_label(static, scaler)
    return [
        common_row(created_at, "summary", "variants_tested", "variants", ", ".join(SCALER_VARIANTS), "", "manual_review_required", label, "Fixed variants only; no parameter search."),
        common_row(created_at, "summary", "codex_strategy_name", CODEX_RISK_CONTROL_STRATEGY, CODEX_RISK_CONTROL_STRATEGY, "", "manual_review_required", label, "Selected to test BTC/ETH core plus a capped alt sleeve with volatility, drawdown, and breadth throttles while preserving at least partial rebound exposure."),
        ...
        common_row(created_at, "summary", "best_scaler_variant", scaler.get("strategy_name", "unavailable"), scaler.get("metric_value", "unavailable"), "", "manual_review_required", label, "Best scaler variant by Calmar."),
        common_row(created_at, "summary", "best_codex_risk_control_variant", CODEX_RISK_CONTROL_STRATEGY, codex.get("metric_value", "unavailable"), "", "manual_review_required", label, "Only one Codex-designed fixed-rule risk-control variant is included."),
        common_row(created_at, "summary", "drawdown_reduction_vs_equal_weight", scaler.get("strategy_name", "unavailable"), round(dd_reduction, 4), "", "manual_review_required", label, "Positive means max drawdown is less severe than static equal-weight."),
        common_row(created_at, "summary", "return_drag_vs_equal_weight", scaler.get("strategy_name", "unavailable"), round(return_drag, 4), "", "manual_review_required", label, "Negative means lower CAGR than static equal-weight."),
        common_row(created_at, "summary", "calmar_improvement_vs_equal_weight", scaler.get("strategy_name", "unavailable"), round(calmar_improvement, 4), "", "manual_review_required", label, "Positive means Calmar improved versus static equal-weight."),
        ...
        common_row(created_at, "summary", "final_summary_label", "crypto_equal_weight_volatility_scaling", label, "", label, label, "Final label is research-only and does not approve execution."),
    ]
...
def insufficient_rows(created_at: str, universe: list[str], universe_note: str, errors: dict[str, str]):
    ...
    summary = [
        common_row(created_at, "summary", "variants_tested", "variants", ", ".join(SCALER_VARIANTS), "", "insufficient_saved_inputs", "insufficient_saved_inputs", evidence),
        common_row(created_at, "summary", "codex_strategy_name", CODEX_RISK_CONTROL_STRATEGY, CODEX_RISK_CONTROL_STRATEGY, "", "insufficient_saved_inputs", "insufficient_saved_inputs", evidence),
        ...
        common_row(created_at, "summary", "best_scaler_variant", "crypto_equal_weight_volatility_scaling", "unavailable", "", "insufficient_saved_inputs", "insufficient_saved_inputs", evidence),
        common_row(created_at, "summary", "best_codex_risk_control_variant", CODEX_RISK_CONTROL_STRATEGY, "unavailable", "", "insufficient_saved_inputs", "insufficient_saved_inputs", evidence),
        common_row(created_at, "summary", "drawdown_reduction_vs_equal_weight", "crypto_equal_weight_volatility_scaling", "unavailable", "", "insufficient_saved_inputs", "insufficient_saved_inputs", evidence),
        common_row(created_at, "summary", "return_drag_vs_equal_weight", "crypto_equal_weight_volatility_scaling", "unavailable", "", "insufficient_saved_inputs", "insufficient_saved_inputs", evidence),
        common_row(created_at, "summary", "calmar_improvement_vs_equal_weight", "crypto_equal_weight_volatility_scaling", "unavailable", "", "insufficient_saved_inputs", "insufficient_saved_inputs", evidence),
        ...
        common_row(created_at, "summary", "final_summary_label", "crypto_equal_weight_volatility_scaling", "insufficient_saved_inputs", "", "insufficient_saved_inputs", "insufficient_saved_inputs", evidence),
    ]
...
def to_common(created_at: str, section: str, metric: dict[str, Any], label: str, period: str | None = None) -> dict[str, Any]:
    metric_value = (
        ...
        f"cash={metric.get('cash_percentage', '')}; avg_exposure={metric.get('average_exposure_pct', '')}; "
        f"trades={metric.get('trade_count', '')}; turnover={metric.get('turnover', '')}; "
        f"avg_hold_days={metric.get('average_holding_period_days', '')}"
    )
    evidence = (
    ...
    if metric.get("strategy_name") == CODEX_RISK_CONTROL_STRATEGY:
        evidence += "; codex_reason=Selected to test BTC/ETH core plus capped alt sleeve with fixed volatility, drawdown, and breadth throttles."
...
def common_row(
    ...
    *,
    ...
    return {
        ...
        "report_name": "crypto_equal_weight_volatility_scaling",
        "section": section,
...
def label_metric(metric: dict[str, Any]) -> str:
    ...
    if metric.get("strategy_name") == CODEX_RISK_CONTROL_STRATEGY:
        if cagr > 15 and calmar > 0 and maxdd > -75:
            return "codex_crypto_risk_control_promising"
        if cagr <= 0 or calmar <= 0:
            return "codex_crypto_risk_control_not_useful"
    if cagr <= 0 or calmar <= 0:
        return "crypto_vol_scaler_not_useful"
    if metric.get("cost_bps") in {25, 50, 100} and cagr < 5:
        return "crypto_vol_scaler_cost_sensitive"
    if maxdd > -70 and cagr > 20:
        return "crypto_vol_scaler_promising"
    if maxdd > -72:
        return "crypto_vol_scaler_drawdown_improved"
...
def label_drawdown(dd: float, static_dd: float, return_drag: float, calmar_improvement: float) -> str:
    ...
    if dd - static_dd < 10:
        return "crypto_vol_scaler_not_useful"
    if return_drag < -35:
        return "crypto_vol_scaler_return_drag_too_high"
    if calmar_improvement > 0:
        return "crypto_vol_scaler_drawdown_improved"
    return "manual_review_required"


def final_label(static: dict[str, Any], scaler: dict[str, Any]) -> str:
    if not static or not scaler:
    ...
    scaler_dd = extract_metric(scaler.get("metric_value", ""), "MaxDD")
    scaler_calmar = extract_metric(scaler.get("metric_value", ""), "Calmar")
    dd_improvement = scaler_dd - static_dd
    cagr_drag = scaler_cagr - static_cagr
    calmar_improvement = scaler_calmar - static_calmar
    if dd_improvement < 10:
        return "equal_weight_still_best_high_drawdown"
    if cagr_drag < -35:
        return "crypto_vol_scaler_return_drag_too_high"
    if dd_improvement >= 10 and calmar_improvement > 0:
        return "crypto_vol_scaler_new_defensive_crypto_candidate"
    if dd_improvement >= 10 and scaler_cagr > 15:
        return "crypto_vol_scaler_drawdown_improved"
    return "manual_review_required"


def best_scaler(rows: list[dict[str, Any]]) -> dict[str, Any]:
    candidates = [row for row in rows if row.get("strategy_name") in SCALER_VARIANTS and row.get("metric_value")]
    if not candidates:
        return {}
...
def to_equity_rows(created_at: str, name: str, cost_bps: int, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        common_row(created_at, "equity_curve", name, "equity", round(float(row.get("equity", 0)), 6), row.get("holdings", ""), "equity_row", "manual_review_required", f"cash_weight={row.get('cash_weight', '')}; average_exposure={round((1.0 - float(row.get('cash_weight', 1.0))) * 100, 4)}", period=row.get("date", ""), cost_bps=cost_bps)
        for row in rows
    ]
...
def build_summary_lines(summary_rows: list[dict[str, Any]], paths: dict[str, Path]) -> list[str]:
    return [
        "Crypto equal-weight volatility-scaling report complete. Research/report only; execution_approved=False.",
        f"Variants tested: {summary_value(summary_rows, 'variants_tested')}",
        f"Codex-designed strategy: {summary_value(summary_rows, 'codex_strategy_name')}",
        ...
        f"Best scaler variant: {summary_value(summary_rows, 'best_scaler_variant')}",
        f"Best Codex risk-control variant: {summary_value(summary_rows, 'best_codex_risk_control_variant')}",
        ...
        "Warning: volatility-scaling report does not approve crypto execution, paper execution, scheduling, or strategy-to-execution wiring.",
    ]
