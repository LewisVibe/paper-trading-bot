...
def vol_executable_ticket_gap_list_status_lines(root: Path) -> list[str]:
    ...
    return [
        ...
        f"- non_submitting_executable_ticket_values_manual_review_decision: {summary_value(rows, 'non_submitting_executable_ticket_values_manual_review_decision')}",
...
def vol_non_submitting_ticket_instance_checkpoint_status_lines(root: Path) -> list[str]:
    ...
    return [
        ...
        f"- review_quantities_created: {summary_value(rows, 'review_quantities_created') or 'False'}",
        f"- review_quantity_estimate_count: {summary_value(rows, 'review_quantity_estimate_count') or '0'}",
        f"- review_quantity_quality_gate_passed: {summary_value(rows, 'review_quantity_quality_gate_passed') or 'False'}",
...
def paper_live_go_no_go_dashboard_status_lines(root: Path) -> list[str]:
    ...
    return [
        ...
        f"- vol_ticket_values_approval_record_decision: {summary_value(rows, 'vol_ticket_values_approval_record_decision')}",
        ...
        f"- vol_review_quantity_estimate_count: {summary_value(rows, 'vol_review_quantity_estimate_count')}",
