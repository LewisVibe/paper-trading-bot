"""Saved-output approval wording checkpoint for approval-criteria blocker closeout.

This defines a simple future human approval phrase for closing only the
approval_criteria_not_approval blocker. It does not record approval, create
...
FINAL_STATUS = "vol_targeted_growth_approval_criteria_closeout_approval_wording_manual_review_required"
FINAL_DECISION = "APPROVAL_CRITERIA_CLOSEOUT_APPROVAL_WORDING_DEFINED_NOT_APPROVED"
NEXT_STEP = "wait_for_explicit_simple_approval_criteria_closeout_approval"

OUTPUT_FILES = {
    "report": Path("data/vol_targeted_growth_executable_ticket_approval_criteria_closeout_approval_wording.csv"),
    "summary": Path("data/vol_targeted_growth_executable_ticket_approval_criteria_closeout_approval_wording_summary.csv"),
    "blockers": Path("data/vol_targeted_growth_executable_ticket_approval_criteria_closeout_approval_wording_blockers.csv"),
    "evidence": Path("data/vol_targeted_growth_executable_ticket_approval_criteria_closeout_approval_wording_evidence.csv"),
}
...
@dataclass
class ApprovalCriteriaCloseoutApprovalWordingResult:
...
def generate_vol_targeted_growth_approval_criteria_closeout_approval_wording(
    root_dir: Path | str = ".",
) -> ApprovalCriteriaCloseoutApprovalWordingResult:
    root = Path(root_dir)
    ...
    return ApprovalCriteriaCloseoutApprovalWordingResult(output_paths, report_rows, summary_rows, blocker_rows, evidence_rows, build_summary_lines(summary_rows, output_paths))


def show_vol_targeted_growth_approval_criteria_closeout_approval_wording(root_dir: Path | str = ".") -> tuple[int, list[str]]:
    summary_path = Path(root_dir) / OUTPUT_FILES["summary"]
    if not summary_path.exists():
        return 1, [
            "Volatility-targeted approval-criteria closeout approval wording is missing.",
            "Run `python bot.py --vol-targeted-growth-approval-criteria-closeout-approval-wording` first.",
            "execution_approved=false; paper_execution_approved=false; scheduling_approved=false",
        ]
    rows = read_csv_rows(summary_path)
    return 0, [
        "Volatility-targeted approval-criteria closeout approval wording saved display. Report only; no approval recorded.",
        ...
        "blocker_closed=false; approval_recorded=false; order_values_populated=false; executable_ticket_created=false; orders_submitted=false; execution_approved=false; paper_execution_approved=false; scheduling_approved=false",
    ]


def build_report_rows() -> list[dict[str, Any]]:
    return [
        row("approval_criteria_closeout_phrase", "approval_wording_defined_not_approved", "high", APPROVAL_PHRASE, "Defines wording for one blocker only.", NEXT_STEP),
        row("approval_boundary", "approval_not_recorded", "critical", "approval_requested=false; approval_recorded=false; blockers_closed=false", "This wording checkpoint is not approval.", "do_not_close_blocker_without_future_explicit_phrase"),
    ]
...
def build_summary_rows(inputs: dict[str, list[dict[str, str]]], report_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    data = [
        ...
        ("final_approval_wording_decision", FINAL_DECISION, "A future phrase is defined, but no approval is recorded."),
        ("active_seed", ACTIVE_SEED, "Current report/status seed."),
        ("active_ticker", ACTIVE_TICKER, "Current report/status ticker label."),
        ("target_blocker", "approval_criteria_not_approval", "Only this blocker could be closed by the future phrase."),
        ...
        ("candidate_review_decision", summary_value(inputs["approval_criteria_candidate"], "final_candidate_review_decision") or "missing_approval_criteria_closeout_candidate_review", "Saved candidate-review decision."),
...
def build_summary_lines(summary_rows: list[dict[str, Any]], output_paths: dict[str, Path]) -> list[str]:
    return [
        "Executable-ticket approval-criteria closeout approval wording complete. Report only; no approval recorded.",
