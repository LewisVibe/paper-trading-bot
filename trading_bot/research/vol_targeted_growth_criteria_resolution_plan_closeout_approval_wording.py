"""Saved-output approval wording checkpoint for criteria resolution-plan closeout.

This report defines a simple future human approval phrase for closing only the
criteria_resolution_plan_open blocker. It does not record approval, close
blockers, create ticket values, create executable tickets, call Alpaca, or
approve execution.
"""
...
FINAL_STATUS = "vol_targeted_growth_criteria_resolution_plan_closeout_approval_wording_manual_review_required"
FINAL_DECISION = "CRITERIA_RESOLUTION_PLAN_CLOSEOUT_APPROVAL_WORDING_DEFINED_NOT_APPROVED"
NEXT_STEP = "wait_for_explicit_simple_criteria_resolution_plan_closeout_approval"

OUTPUT_FILES = {
    "report": Path("data/vol_targeted_growth_executable_ticket_criteria_resolution_plan_closeout_approval_wording.csv"),
    "summary": Path("data/vol_targeted_growth_executable_ticket_criteria_resolution_plan_closeout_approval_wording_summary.csv"),
    "blockers": Path("data/vol_targeted_growth_executable_ticket_criteria_resolution_plan_closeout_approval_wording_blockers.csv"),
    "evidence": Path("data/vol_targeted_growth_executable_ticket_criteria_resolution_plan_closeout_approval_wording_evidence.csv"),
}
...
@dataclass
class CriteriaResolutionPlanCloseoutApprovalWordingResult:
...
def generate_vol_targeted_growth_criteria_resolution_plan_closeout_approval_wording(
    root_dir: Path | str = ".",
) -> CriteriaResolutionPlanCloseoutApprovalWordingResult:
    root = Path(root_dir)
    ...
    return CriteriaResolutionPlanCloseoutApprovalWordingResult(
        output_paths,
        report_rows,
        summary_rows,
        blocker_rows,
...
def show_vol_targeted_growth_criteria_resolution_plan_closeout_approval_wording(
    root_dir: Path | str = ".",
) -> tuple[int, list[str]]:
    summary_path = Path(root_dir) / OUTPUT_FILES["summary"]
    if not summary_path.exists():
        return 1, [
            "Volatility-targeted criteria resolution-plan closeout approval wording is missing.",
            "Run `python bot.py --vol-targeted-growth-criteria-resolution-plan-closeout-approval-wording` first.",
            "execution_approved=false; paper_execution_approved=false; scheduling_approved=false",
        ]
    rows = read_csv_rows(summary_path)
    return 0, [
        "Volatility-targeted criteria resolution-plan closeout approval wording saved display. Report only; no approval recorded.",
...
def build_context(inputs: dict[str, list[dict[str, str]]]) -> dict[str, str]:
    return {
        "candidate_review_decision": summary_value(inputs["resolution_plan_candidate"], "final_candidate_review_decision") or "missing_resolution_plan_closeout_candidate_review",
        "candidate_review_status": summary_value(inputs["resolution_plan_candidate"], "final_candidate_review_status") or "missing_resolution_plan_closeout_candidate_review",
...
def build_report_rows(context: dict[str, str]) -> list[dict[str, Any]]:
    return [
        report_row(
            "criteria_resolution_plan_closeout_phrase",
            "approval_wording_defined_not_approved",
            "high",
            APPROVAL_PHRASE,
            "A short future approval phrase is defined for only the resolution-plan blocker.",
            NEXT_STEP,
        ),
        report_row(
            "criteria_source_prior_closeout",
            "prior_closeout_required_context",
            "high",
            context["criteria_source_closeout_decision"],
            "The second closeout should only be interpreted after the criteria-source closeout record exists.",
            "review_prior_closeout_before_resolution_plan_closeout",
        ),
        report_row(
            "approval_boundary",
...
def build_summary_rows(context: dict[str, str], report_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows = [
        ...
        ("target_blocker", "criteria_resolution_plan_open", "Only this blocker could be closed by the future phrase."),
...
def build_blocker_rows(inputs: dict[str, list[dict[str, str]]], context: dict[str, str]) -> list[dict[str, Any]]:
    rows = [
        ...
        blocker_row("criteria_resolution_plan_blocker_not_closed", "blocked", "critical", "blocker_closed=false; blockers_closed=false", "future_explicit_approval_required"),
...
def build_summary_lines(summary_rows: list[dict[str, Any]], output_paths: dict[str, Path]) -> list[str]:
    return [
        "Executable-ticket criteria resolution-plan closeout approval wording complete. Report only; no approval recorded.",
        ...
        f"criteria_source_closeout_decision={summary_value(summary_rows, 'criteria_source_closeout_decision')}",
