...
Strategy improvement diagnostics mode reads the saved lab and robustness CSVs only and explains why `growth_biased_rotation_crash_gate` is split-sensitive. It checks split decay, benchmark-relative gaps, cost sensitivity, drawdown window/recovery context, cash drag, and whether the candidate remains the active research lead despite trailing SPY. It also compares `growth_biased_rotation_cost_aware_rebalance` and `growth_biased_rotation_partial_defensive_sleeve` directly against the original growth-biased version. The cost-aware and partial defensive-sleeve refinements remain in the lab, but diagnostics treat them as tested/rejected when they show return drag without improving cost or split sensitivity. The next suggestion-only hypotheses are now re-entry filtering, split-stability checking, regime-recovery filtering, and breadth-threshold review. It does not add another strategy, rerun yfinance-heavy backtests, call Alpaca, create orders, schedule anything, or approve execution.
