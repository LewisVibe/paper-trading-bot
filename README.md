...
Volatility-targeted growth executable ticket gap list is saved-output/status-only: `python bot.py --vol-targeted-growth-executable-ticket-gap-list` and `python bot.py --show-vol-targeted-growth-executable-ticket-gap-list` turn the remaining blocker chain into a concise gap list. It keeps the final decision at `EXECUTABLE_TICKET_DESIGN_NOT_READY`; no broker read, order field, executable ticket, paper execution, live trading, or scheduling is approved.
