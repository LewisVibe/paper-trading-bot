...
Volatility-targeted growth executable ticket gap list is saved-output/status-only: `python bot.py --vol-targeted-growth-executable-ticket-gap-list` and `python bot.py --show-vol-targeted-growth-executable-ticket-gap-list` turn the remaining blocker chain into a concise gap list. The VPS daily monitoring summary now surfaces this gap-list status when saved output exists. It keeps the final decision at `EXECUTABLE_TICKET_DESIGN_NOT_READY`; no broker read, order field, executable ticket, paper execution, live trading, or scheduling is approved.

Volatility-targeted growth manual execution-design approval gate is saved-output/report-only: `python bot.py --vol-targeted-growth-manual-execution-design-approval-gate` and `python bot.py --show-vol-targeted-growth-manual-execution-design-approval-gate` define what a future explicit approval prompt would need to say before non-submitting executable ticket design could be discussed. The VPS daily monitoring summary now surfaces this approval-gate status when saved output exists. The current decision is `MANUAL_EXECUTION_DESIGN_APPROVAL_NOT_RECORDED`; it does not record approval, create order fields, create executable tickets, submit orders, approve execution, or approve scheduling.

Volatility-targeted growth non-submitting ticket schema design is saved-output/design-only: `python bot.py --vol-targeted-growth-non-submitting-ticket-schema-design` and `python bot.py --show-vol-targeted-growth-non-submitting-ticket-schema-design` define the future ticket schema shape after explicit user approval for design work. The VPS daily monitoring summary surfaces this schema-design status when saved output exists. It keeps `ticket_instance_created=False`, `order_values_populated=False`, and `NON_SUBMITTING_TICKET_SCHEMA_DESIGNED_NO_TICKET_CREATED`; no ticket instance, order instruction, broker read, order submission, execution approval, or scheduling approval is created.

Volatility-targeted growth non-submitting ticket-instance design is saved-output/design-only: `python bot.py --vol-targeted-growth-non-submitting-ticket-instance-design` and `python bot.py --show-vol-targeted-growth-non-submitting-ticket-instance-design` create a reviewed draft ticket shape without making it executable. The VPS daily monitoring summary surfaces this status when saved output exists. It keeps `ticket_instance_created=False`, `executable_ticket_created=False`, `order_values_populated=False`, and `NON_SUBMITTING_TICKET_INSTANCE_DESIGNED_NO_ORDER_VALUES`; no side, quantity, order type, account reference, broker order id, broker read, order submission, execution approval, or scheduling approval is created.

Volatility-targeted growth fresh broker pre-ticket gate design is saved-output/design-only: `python bot.py --vol-targeted-growth-fresh-broker-pre-ticket-gate-design` and `python bot.py --show-vol-targeted-growth-fresh-broker-pre-ticket-gate-design` define the future read-only broker gate that would be needed before any ticket values could be discussed. The VPS daily monitoring summary surfaces this status when saved output exists. It keeps `fresh_broker_pre_ticket_gate_run=False`, `readonly_alpaca_check_run=False`, `broker_positions_read=False`, `order_values_populated=False`, and `FRESH_BROKER_PRE_TICKET_GATE_DESIGNED_NOT_RUN`; no Alpaca call, broker read, order value, order instruction, execution approval, or scheduling approval is created.

Volatility-targeted growth fresh broker pre-ticket gate run-readiness is saved-output/readiness-only: `python bot.py --vol-targeted-growth-fresh-broker-pre-ticket-gate-run-readiness` and `python bot.py --show-vol-targeted-growth-fresh-broker-pre-ticket-gate-run-readiness` check whether the saved design chain is ready to ask for a future explicit read-only Alpaca broker gate run. The VPS daily monitoring summary surfaces this status when saved output exists. It may set `ready_to_request_readonly_approval=True`, but keeps `readonly_alpaca_run_approved=False`, `fresh_broker_pre_ticket_gate_run=False`, `broker_positions_read=False`, `order_values_populated=False`, and all execution/scheduling approvals false.

Volatility-targeted growth fresh broker pre-ticket gate run is explicitly confirmed/read-only: `python bot.py --vol-targeted-growth-fresh-broker-pre-ticket-gate-run --confirm-readonly-alpaca-check` may read Alpaca paper positions once after manual approval and saves that broker context for review. The display command is `python bot.py --show-vol-targeted-growth-fresh-broker-pre-ticket-gate-run`. This is still not a ticket, not an order, and not execution approval: it keeps `ticket_instance_created=False`, `order_values_populated=False`, `orders_submitted=False`, `execution_approved=False`, `paper_execution_approved=False`, and `scheduling_approved=False`. Without the confirmation flag, the command writes a missing-confirmation report and does not read the broker.

Volatility-targeted growth post-gate review is saved-output-only: `python bot.py --vol-targeted-growth-post-gate-review` and `python bot.py --show-vol-targeted-growth-post-gate-review` interpret the saved fresh broker gate result without calling Alpaca again. The paper-live go/no-go dashboard now surfaces this post-gate status. A successful review can confirm saved broker context exists, but it still keeps `ticket_values_not_approved_after_readonly_context`, `order_values_populated=False`, `orders_submitted=False`, `execution_approved=False`, `paper_execution_approved=False`, and `scheduling_approved=False`.

Volatility-targeted growth manual ticket-value design is saved-output/design-only: `python bot.py --vol-targeted-growth-manual-ticket-value-design` and `python bot.py --show-vol-targeted-growth-manual-ticket-value-design` list the future ticket fields that would need explicit manual values later. It keeps side, quantity, order type, time-in-force, account reference, and broker order id blank/blocked, with `populated_ticket_value_count=0`, `order_values_populated=False`, `order_instructions_created=False`, `executable_ticket_created=False`, and all execution/scheduling approvals false. The paper-live go/no-go dashboard surfaces this status but remains `NO_GO_EXECUTION_BLOCKED_MONITOR_ONLY`.

Volatility-targeted growth executable-ticket prerequisites closeout is saved-output/report-only: `python bot.py --vol-targeted-growth-executable-ticket-prerequisites-closeout` and `python bot.py --show-vol-targeted-growth-executable-ticket-prerequisites-closeout` record `EXECUTABLE_TICKET_PREREQUISITES_NOT_CLOSED`. The approval-readiness checkpoint, `python bot.py --vol-targeted-growth-executable-ticket-approval-readiness`, records `NOT_READY_TO_REQUEST_EXECUTABLE_TICKET_APPROVAL`. The approval-criteria checkpoint, `python bot.py --vol-targeted-growth-executable-ticket-approval-criteria`, defines what would need manual review later and records `APPROVAL_CRITERIA_DEFINED_APPROVAL_NOT_REQUESTED`. The criteria resolution plan, `python bot.py --vol-targeted-growth-executable-ticket-criteria-resolution-plan`, orders those manual blockers and records `CRITERIA_BLOCKER_RESOLUTION_PLAN_CREATED_APPROVAL_STILL_BLOCKED`. The criteria source review, `python bot.py --vol-targeted-growth-executable-ticket-criteria-source-review`, records `CRITERIA_SOURCE_REVIEWED_NO_BLOCKERS_CLOSED`. The blocker closeout review, `python bot.py --vol-targeted-growth-executable-ticket-criteria-blocker-closeout-review`, records `CRITERIA_BLOCKERS_REVIEWED_NONE_CLOSED`. The first blocker-specific reviews, including `python bot.py --vol-targeted-growth-criteria-blocker-specific-review-rollup`, record `CRITERIA_BLOCKER_SPECIFIC_REVIEWS_CREATED_NONE_CLOSED`. The closeout-candidate rollup, `python bot.py --vol-targeted-growth-criteria-closeout-candidate-review-rollup`, records `CRITERIA_CLOSEOUT_CANDIDATES_REVIEWED_NONE_CLOSED`. The criteria-source closeout approval wording checkpoint, `python bot.py --vol-targeted-growth-criteria-source-closeout-approval-wording`, stores the simple future phrase `I approve closing the criteria_source_reviewed blocker only.` and records `CRITERIA_SOURCE_CLOSEOUT_APPROVAL_WORDING_DEFINED_NOT_APPROVED`. The criteria-source closeout record, `python bot.py --vol-targeted-growth-criteria-source-closeout-record`, records `CRITERIA_SOURCE_REVIEWED_BLOCKER_CLOSED_ONLY` for that one blocker. The criteria-resolution-plan closeout wording checkpoint and record, `python bot.py --vol-targeted-growth-criteria-resolution-plan-closeout-approval-wording` and `python bot.py --vol-targeted-growth-criteria-resolution-plan-closeout-record`, close only `criteria_resolution_plan_open`. The approval-criteria closeout wording checkpoint and record, `python bot.py --vol-targeted-growth-approval-criteria-closeout-approval-wording` and `python bot.py --vol-targeted-growth-approval-criteria-closeout-record`, close only `approval_criteria_not_approval`. The final ticket blockers closeout wording and record, `python bot.py --vol-targeted-growth-final-ticket-blockers-closeout-approval-wording` and `python bot.py --vol-targeted-growth-final-ticket-blockers-closeout-record`, close `ticket_values_not_approved` and `executable_ticket_prerequisites_not_met` as checklist blockers only; they still keep order values blank, no executable ticket created, and execution/scheduling false.

Volatility-targeted growth paper-live execution blocker rollup is saved-output/status-only: `python bot.py --vol-targeted-growth-paper-live-execution-blocker-rollup` and `python bot.py --show-vol-targeted-growth-paper-live-execution-blocker-rollup` summarize the whole volatility paper-live chain from manual gate through ticket prerequisites. The rollup and `python bot.py --vol-targeted-growth-executable-ticket-gap-list` now recognise five saved checklist blocker closeouts, surfacing `closed_blocker_count=5` and `remaining_known_blockers_after_closeout=none`. This still does not approve execution: no executable ticket exists, order values are not populated, and execution/scheduling stay false.

Volatility-targeted growth execution approval request readiness is saved-output/report-only: `python bot.py --vol-targeted-growth-execution-approval-request-readiness` and `python bot.py --show-vol-targeted-growth-execution-approval-request-readiness` can report `READY_FOR_SEPARATE_EXECUTION_APPROVAL_REQUEST_NOT_APPROVED` after the checklist blockers are closed. This means the project is ready to ask a separate human approval question only; it keeps `approval_requested=False`, `approval_recorded=False`, `order_values_populated=False`, `executable_ticket_created=False`, `orders_submitted=False`, `execution_approved=False`, `paper_execution_approved=False`, and `scheduling_approved=False`.

Volatility-targeted growth execution-design approval is saved-output/design-only: `python bot.py --vol-targeted-growth-execution-design-approval-wording`, `python bot.py --show-vol-targeted-growth-execution-design-approval-wording`, `python bot.py --vol-targeted-growth-execution-design-approval-record`, and `python bot.py --show-vol-targeted-growth-execution-design-approval-record` define and record approval to design the next non-submitting execution-ticket layer only. This still keeps order values blank, no executable ticket created, no orders submitted, and `execution_approved=False`, `paper_execution_approved=False`, and `scheduling_approved=False`.

Volatility-targeted growth non-submitting executable-ticket design is saved-output/design-only: `python bot.py --vol-targeted-growth-non-submitting-executable-ticket-design` and `python bot.py --show-vol-targeted-growth-non-submitting-executable-ticket-design` create the first post-approval ticket design artifact while still keeping every order value blank. It may mark `executable_ticket_design_created=True`, but keeps `executable_ticket_created=False`, `order_values_populated=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth ticket-values discussion approval is saved-output/report-only: `python bot.py --vol-targeted-growth-ticket-values-approval-readiness`, `python bot.py --vol-targeted-growth-ticket-values-approval-wording`, and `python bot.py --vol-targeted-growth-ticket-values-approval-record` allow the next discussion to move toward non-executable value placeholders only. The record may set `ticket_value_discussion_approved=True`, but it keeps `ticket_values_approved=False`, `order_values_populated=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth ticket-value placeholders and quality gate are saved-output/report-only: `python bot.py --vol-targeted-growth-ticket-value-placeholders`, `python bot.py --show-vol-targeted-growth-ticket-value-placeholders`, `python bot.py --vol-targeted-growth-ticket-value-quality-gate`, and `python bot.py --show-vol-targeted-growth-ticket-value-quality-gate` create blank placeholders and verify they remain non-executable. A passing quality gate means the placeholder structure is usable for manual review only; it still keeps `ticket_values_approved=False`, `order_values_populated=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth ticket-value proposal approval is saved-output/report-only: `python bot.py --vol-targeted-growth-ticket-value-proposal-approval-wording`, `python bot.py --show-vol-targeted-growth-ticket-value-proposal-approval-wording`, `python bot.py --vol-targeted-growth-ticket-value-proposal-approval-record`, and `python bot.py --show-vol-targeted-growth-ticket-value-proposal-approval-record` record permission to draft proposed ticket values in a future review-only report. The record may set `ticket_value_proposal_discussion_approved=True`, but it keeps `proposed_ticket_values_created=False`, `ticket_values_approved=False`, `order_values_populated=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth proposed ticket values are saved-output/review-only: `python bot.py --vol-targeted-growth-proposed-ticket-values`, `python bot.py --show-vol-targeted-growth-proposed-ticket-values`, `python bot.py --vol-targeted-growth-proposed-ticket-values-quality-gate`, and `python bot.py --show-vol-targeted-growth-proposed-ticket-values-quality-gate` draft non-executable proposal labels for manual review. The quality gate can pass with `proposed_ticket_values_created=True`, but it keeps `ticket_values_approved=False`, `order_values_populated=False`, `order_instructions_created=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth executable-ticket draft readiness is saved-output/readiness-only: `python bot.py --vol-targeted-growth-executable-ticket-draft-readiness` and `python bot.py --show-vol-targeted-growth-executable-ticket-draft-readiness` review whether the non-executable proposed values are clear enough for a later manual draft discussion. A ready result can set `draft_discussion_ready=True`, but it still creates no ticket, no side, no quantity, no order instructions, no broker call, and no execution/scheduling approval.

Volatility-targeted growth non-submitting executable-ticket draft is saved-output/draft-only: `python bot.py --vol-targeted-growth-non-submitting-executable-ticket-draft`, `python bot.py --show-vol-targeted-growth-non-submitting-executable-ticket-draft`, `python bot.py --vol-targeted-growth-non-submitting-executable-ticket-draft-quality-gate`, and `python bot.py --show-vol-targeted-growth-non-submitting-executable-ticket-draft-quality-gate` create a review draft from the proposed labels and verify it still has no executable order fields. A passing draft quality gate can set `draft_ticket_created=True`, but it keeps `ticket_values_approved=False`, `order_values_populated=False`, `order_instructions_created=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth draft ticket-value approval readiness is saved-output/readiness-only: `python bot.py --vol-targeted-growth-draft-ticket-value-approval-readiness` and `python bot.py --show-vol-targeted-growth-draft-ticket-value-approval-readiness` check whether the non-submitting draft is complete enough to ask later for explicit ticket-value approval. A ready result can set `ticket_value_approval_request_ready=True`, but it keeps `ticket_value_approval_requested=False`, `ticket_value_approval_recorded=False`, `ticket_values_approved=False`, `order_values_populated=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth draft ticket-value approval wording/record is saved-output/report-only: `python bot.py --vol-targeted-growth-draft-ticket-value-approval-wording`, `python bot.py --show-vol-targeted-growth-draft-ticket-value-approval-wording`, `python bot.py --vol-targeted-growth-draft-ticket-value-approval-record`, and `python bot.py --show-vol-targeted-growth-draft-ticket-value-approval-record` define and record narrow approval to populate review-only draft ticket values in a later step. The record can set `ticket_value_population_approved=True`, but it keeps `ticket_values_approved=False`, `order_values_populated=False`, `order_instructions_created=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth review-only draft ticket values are saved-output/report-only: `python bot.py --vol-targeted-growth-review-only-draft-ticket-values`, `python bot.py --show-vol-targeted-growth-review-only-draft-ticket-values`, `python bot.py --vol-targeted-growth-review-only-draft-ticket-values-quality-gate`, and `python bot.py --show-vol-targeted-growth-review-only-draft-ticket-values-quality-gate` populate review labels such as sleeve targets and blocked side/quantity fields. A passing quality gate can set `draft_ticket_values_created=True`, but it keeps `ticket_values_approved=False`, `order_values_populated=False`, `order_instructions_created=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth draft ticket-values manual review/readiness is saved-output/report-only: `python bot.py --vol-targeted-growth-draft-ticket-values-manual-review`, `python bot.py --show-vol-targeted-growth-draft-ticket-values-manual-review`, `python bot.py --vol-targeted-growth-executable-ticket-values-readiness`, and `python bot.py --show-vol-targeted-growth-executable-ticket-values-readiness` review the saved draft labels and can mark a future executable-values approval request as ready. Readiness can set `executable_ticket_values_approval_request_ready=True`, but it keeps `executable_ticket_values_approval_requested=False`, `executable_ticket_values_approved=False`, `order_values_populated=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth executable ticket-values approval is saved-output/report-only: `python bot.py --vol-targeted-growth-executable-ticket-values-approval-wording`, `python bot.py --show-vol-targeted-growth-executable-ticket-values-approval-wording`, `python bot.py --vol-targeted-growth-executable-ticket-values-approval-record`, and `python bot.py --show-vol-targeted-growth-executable-ticket-values-approval-record` record narrow approval for a later non-submitting value population step only. The record can set `executable_ticket_values_approved=True` and `ticket_values_approved=True`, but it keeps `order_values_populated=False`, `order_instructions_created=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth non-submitting executable ticket values are saved-output/report-only: `python bot.py --vol-targeted-growth-non-submitting-executable-ticket-values`, `python bot.py --show-vol-targeted-growth-non-submitting-executable-ticket-values`, `python bot.py --vol-targeted-growth-non-submitting-executable-ticket-values-quality-gate`, and `python bot.py --show-vol-targeted-growth-non-submitting-executable-ticket-values-quality-gate` populate reviewable seed/sleeve context after the explicit approval record. When saved review quantity estimates exist, they are carried into the packet as review-only estimates, not broker-ready order quantities. The quality gate keeps `broker_ready_order_values_populated=False`, `order_values_populated=False`, `order_instructions_created=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth non-submitting values manual review and ticket-creation readiness are saved-output/report-only: `python bot.py --vol-targeted-growth-non-submitting-executable-ticket-values-manual-review`, `python bot.py --show-vol-targeted-growth-non-submitting-executable-ticket-values-manual-review`, `python bot.py --vol-targeted-growth-non-submitting-ticket-creation-readiness`, and `python bot.py --show-vol-targeted-growth-non-submitting-ticket-creation-readiness` record that the populated values were reviewed and that a future non-submitting ticket-instance discussion may be considered. Readiness can set `ticket_creation_discussion_ready=True`, but it keeps `ticket_creation_approved=False`, `ticket_instance_created=False`, `executable_ticket_created=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth non-submitting ticket-instance checkpoint is saved-output/report-only: `python bot.py --vol-targeted-growth-non-submitting-ticket-instance-checkpoint` and `python bot.py --show-vol-targeted-growth-non-submitting-ticket-instance-checkpoint` create a review checkpoint after readiness without making it broker-ready. When saved review quantities exist, they are carried into the checkpoint as manual-review context only. It can set `ticket_instance_checkpoint_created=True`, but it keeps `ticket_instance_created=False`, `ticket_creation_approved=False`, `broker_ready_order_values_populated=False`, `order_values_populated=False`, `order_instructions_created=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth non-submitting ticket-instance quality gate is saved-output/report-only: `python bot.py --vol-targeted-growth-non-submitting-ticket-instance-quality-gate` and `python bot.py --show-vol-targeted-growth-non-submitting-ticket-instance-quality-gate` check that the enriched checkpoint has complete review inputs, blank protected order fields, and no broker-ready order values. Passing this gate only means the packet is clean for manual review; it still keeps `ticket_instance_created=False`, `executable_ticket_created=False`, `broker_ready_order_values_populated=False`, `order_values_populated=False`, `order_instructions_created=False`, `orders_submitted=False`, and all execution/scheduling approvals false.

Volatility-targeted growth sleeve mapping and action proposal are saved-output/report-only: `python bot.py --vol-targeted-growth-sleeve-symbol-mapping`, `python bot.py --show-vol-targeted-growth-sleeve-symbol-mapping`, `python bot.py --vol-targeted-growth-broker-ready-action-proposal`, and `python bot.py --show-vol-targeted-growth-broker-ready-action-proposal` map the active multi-sleeve seed to review symbols `QQQ`, `MGK`, `IBIT`, and `SGOV`. The proposal keeps side, quantity, order type, time-in-force, account, broker order id, order instructions, order submission, execution approval, and scheduling approval absent/false.

Volatility-targeted calculated order values are saved-output/report-only: `python bot.py --vol-targeted-growth-calculated-order-values` and `python bot.py --show-vol-targeted-growth-calculated-order-values` calculate target-dollar review values from the mapped symbols using a conservative `$1000` review notional. Share quantities, side, order type, time-in-force, account, broker order id, order instructions, order submission, execution approval, and scheduling approval remain absent/false until saved prices and separate explicit approval exist.

Volatility-targeted saved-price snapshot readiness is saved-output/report-only: `python bot.py --vol-targeted-growth-saved-price-snapshot-readiness` and `python bot.py --show-vol-targeted-growth-saved-price-snapshot-readiness` define the price evidence that would be required before target-dollar values could become share quantities. They do not fetch prices, call Alpaca, refresh yfinance data, create order instructions, approve execution, or approve scheduling.

Volatility-targeted saved-price snapshot approval wording/record are saved-output/report-only: `python bot.py --vol-targeted-growth-saved-price-snapshot-approval-wording`, `python bot.py --show-vol-targeted-growth-saved-price-snapshot-approval-wording`, `python bot.py --vol-targeted-growth-saved-price-snapshot-approval-record`, and `python bot.py --show-vol-targeted-growth-saved-price-snapshot-approval-record` define and record approval to discuss the future saved-price snapshot method only. They do not approve a price fetch, saved prices, quantity calculation, order instructions, execution, or scheduling.

Volatility-targeted saved-price snapshot runner design is saved-output/report-only: `python bot.py --vol-targeted-growth-saved-price-snapshot-runner-design` and `python bot.py --show-vol-targeted-growth-saved-price-snapshot-runner-design` define allowed future snapshot fields, stale-price rules, provider-failure handling, and stop conditions. They do not implement a price snapshot runner, fetch prices, calculate quantities, create order instructions, approve execution, or approve scheduling.

Volatility-targeted saved-price snapshot runner readiness is saved-output/report-only: `python bot.py --vol-targeted-growth-saved-price-snapshot-runner-readiness` and `python bot.py --show-vol-targeted-growth-saved-price-snapshot-runner-readiness` check whether the saved design, method-discussion approval, readiness, and calculated target-dollar context are complete enough to discuss a future runner implementation. They do not implement the runner, fetch prices, calculate quantities, create order instructions, approve execution, or approve scheduling.

Volatility-targeted saved-price snapshot runner implementation approval wording/record are saved-output/report-only: `python bot.py --vol-targeted-growth-saved-price-snapshot-runner-approval-wording`, `python bot.py --show-vol-targeted-growth-saved-price-snapshot-runner-approval-wording`, `python bot.py --vol-targeted-growth-saved-price-snapshot-runner-approval-record`, and `python bot.py --show-vol-targeted-growth-saved-price-snapshot-runner-approval-record` define and record approval to implement the future runner only. They do not run it, fetch prices, calculate quantities, create order instructions, approve execution, or approve scheduling.

Volatility-targeted saved-price snapshot runner is guarded by default: `python bot.py --vol-targeted-growth-saved-price-snapshot` writes a blocked report without fetching prices. A future confirmed run requires `--confirm-saved-price-snapshot-run`; even then it only saves price fields and still does not calculate quantities, create order instructions, call Alpaca, approve execution, or approve scheduling.

Volatility-targeted saved-price snapshot run approval wording/record are saved-output/report-only: `python bot.py --vol-targeted-growth-saved-price-snapshot-run-approval-wording`, `python bot.py --show-vol-targeted-growth-saved-price-snapshot-run-approval-wording`, `python bot.py --vol-targeted-growth-saved-price-snapshot-run-approval-record`, and `python bot.py --show-vol-targeted-growth-saved-price-snapshot-run-approval-record` define and record approval for one future price-only snapshot run. They do not fetch prices themselves, calculate quantities, create order instructions, call Alpaca, approve execution, or approve scheduling.

Volatility-targeted saved-price snapshot quality gate is saved-output/report-only: `python bot.py --vol-targeted-growth-saved-price-snapshot-quality-gate` and `python bot.py --show-vol-targeted-growth-saved-price-snapshot-quality-gate` check whether the saved snapshot has all required symbols, usable positive prices, no price errors, and fresh timestamps. A passing quality gate is still manual-review-only and keeps quantity calculation, order instructions, Alpaca, execution, and scheduling blocked.

Volatility-targeted quantity-calculation readiness is saved-output/report-only: `python bot.py --vol-targeted-growth-quantity-calculation-readiness` and `python bot.py --show-vol-targeted-growth-quantity-calculation-readiness` check whether saved target dollars and saved price quality are complete enough to request a future quantity-calculation approval record. It does not calculate shares, create order instructions, call Alpaca, approve execution, or approve scheduling.

Volatility-targeted review quantity estimates are saved-output/report-only: `python bot.py --vol-targeted-growth-quantity-calculation-approval-wording`, `python bot.py --vol-targeted-growth-quantity-calculation-approval-record`, `python bot.py --vol-targeted-growth-review-quantity-estimates`, and `python bot.py --vol-targeted-growth-review-quantity-quality-gate` define approval, create saved review-only share quantity estimates, and quality-check those estimates. These quantities are not order quantities; no side, order type, account, ticket, Alpaca call, execution approval, or scheduling approval is created.

Paper-live go/no-go dashboard is saved-output/status-only: `python bot.py --paper-live-go-no-go-dashboard` and `python bot.py --show-paper-live-go-no-go-dashboard` summarize QQQ100 no-action state, the active volatility seed blocker rollup, review quantity estimate state, executable-ticket closeout/readiness/criteria/resolution/source-review/blocker-review chain, paper-live checklist phase, and VPS monitoring assumptions in one view. The expected current decision is `NO_GO_EXECUTION_BLOCKED_MONITOR_ONLY`; it does not call Alpaca, create order instructions, create executable tickets, approve execution, or approve scheduling.
