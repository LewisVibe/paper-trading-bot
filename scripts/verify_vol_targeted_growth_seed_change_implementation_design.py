...
from trading_bot.research.vol_targeted_growth_seed_change_implementation_design import (  # noqa: E402
    FINAL_STATUS,
    OUTPUT_FILES,
    SAFETY_FLAGS,
...
def main() -> int:
    ...
    module_source = read_text(ROOT / "trading_bot" / "research" / "vol_targeted_growth_seed_change_implementation_design.py")
    verify_commands(bot_source, failures)
    verify_outputs_ignored(failures)
    verify_source(module_source, failures)
    verify_fixture(failures)
    if failures:
        print("Volatility-targeted growth seed-change implementation design verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Volatility-targeted growth seed-change implementation design verification passed.")
    return 0
...
def verify_source(source: str, failures: list[str]) -> None:
    for token in [
        FINAL_STATUS,
        "manual_review_seed_change_implementation_design_before_code_change",
        "implementation_design_created_no_code_change_applied",
        "seed_not_changed_qqq100_retained_until_separate_implementation",
        "separate_seed_change_code_change_required",
    ...
    show_body = source_slice(source, "def show_vol_targeted_growth_seed_change_implementation_design", "def manual_approval_ready")
    if "write_rows" in show_body or "generate_vol_targeted_growth_seed_change_implementation_design" in show_body:
...
def verify_fixture(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        seed_inputs(root)
        result = generate_vol_targeted_growth_seed_change_implementation_design(root)
        ...
        if summary_value(result.summary_rows, "implementation_scope") != "design_seed_change_implementation_without_execution":
            failures.append("implementation scope must remain design-only")
        ...
        if summary_value(result.summary_rows, "implementation_status") != "implementation_design_created_no_code_change_applied":
            failures.append("implementation design must not apply the code change")
        if summary_value(result.summary_rows, "largest_blocker") != "separate_seed_change_code_change_required":
            failures.append("largest blocker should require a separate code change")
        for row in result.summary_rows + result.design_rows + result.evidence_rows + result.blocker_rows:
            ...
            for flag in TRUE_FLAGS:
                if str(row.get(flag, "")).lower() != "true":
                    ...
                    return
        code, lines = show_vol_targeted_growth_seed_change_implementation_design(root)
        joined = "\n".join(lines)
        if code != 0 or FINAL_STATUS not in joined:
            failures.append("show command did not display saved implementation design")
...
def seed_inputs(root: Path) -> None:
    ...
    data.mkdir()
    write_summary(
        data / "vol_targeted_growth_seed_change_manual_approval_summary.csv",
        {
            "final_manual_approval_status": "vol_targeted_growth_seed_change_manual_approval_recorded_implementation_required",
            "manual_approval_scope": "approval_to_design_seed_change_implementation_not_to_execute",
            "seed_change_decision": "seed_not_changed_qqq100_retained_until_separate_implementation",
            "implementation_status": "implementation_not_added",
        },
    )
