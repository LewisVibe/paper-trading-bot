...
BOT = ROOT / "bot.py"
MODULE = ROOT / "trading_bot" / "research" / "high_growth_stock_manual_review_pack.py"
README = ROOT / "README.md"
CURRENT_STATE = ROOT / "docs" / "CURRENT_STATE.md"
V2_CHECKPOINT = ROOT / "docs" / "V2_RESEARCH_CHECKPOINT.md"
HERMES_TASK_BOARD = ROOT / "docs" / "HERMES_TASK_BOARD.md"

COMMANDS = ["--high-growth-stock-manual-review-pack", "--show-high-growth-stock-manual-review-pack"]
OUTPUTS = [
...
REQUIRED_TOKENS = [
    "high_growth_stock_branch_manual_review_required",
    "high_growth_stock_branch_research_only_confirmed",
    "clean_main_lead_unchanged",
    "high_risk_stock_lead_candidate_confirmed",
    "extreme_drawdown_reference_rejected",
...
def main() -> int:
    ...
    if failures:
        print("High-growth stock manual review pack verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("High-growth stock manual review pack verification passed.")
    print("Verified saved-output-only review pack, clean lead retention, high-risk research-only label, rejected Top1 reference, ignored outputs, and non-execution boundaries.")
    return 0
...
def verify_commands(bot_source: str, failures: list[str]) -> None:
    ...
    for command, branch in [
        ("--high-growth-stock-manual-review-pack", 'if sys.argv[1:] == ["--high-growth-stock-manual-review-pack"]:'),
        ("--show-high-growth-stock-manual-review-pack", 'if sys.argv[1:] == ["--show-high-growth-stock-manual-review-pack"]:'),
    ]:
...
def verify_module(module_source: str, failures: list[str]) -> None:
    ...
    if "read_csv_rows" not in module_source:
        failures.append("manual review pack should read saved CSV outputs only")
    display_start = module_source.find("def show_high_growth_stock_manual_review_pack")
    if display_start == -1:
        failures.append("missing saved-display function")
    else:
        display_source = module_source[display_start:]
        if "generate_high_growth_stock_manual_review_pack" in display_source:
...
def verify_docs(docs_source: str, failures: list[str]) -> None:
    ...
    for phrase in ["high-growth stock manual review pack", "saved-output", "research-only", "does not approve execution"]:
