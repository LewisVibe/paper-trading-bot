"""Verify execution-design approval stays design-only and report-only."""
...
from trading_bot.research.vol_targeted_growth_execution_design_approval import (  # noqa: E402
    APPROVAL_PHRASE,
    RECORD_DECISION,
    RECORD_OUTPUTS,
    RECORD_STATUS,
...
def main() -> int:
    ...
    if failures:
        print("Execution design approval verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Execution design approval verification passed.")
    print("Verified design-only approval record, false execution flags, and no broker/order/scheduling calls.")
    return 0
...
def verify_source_safety(failures: list[str]) -> None:
    source = (ROOT / "trading_bot/research/vol_targeted_growth_execution_design_approval.py").read_text(encoding="utf-8")
    for phrase in [
        "TradingClient(",
        "MarketOrderRequest(",
        "submit_order",
...
def verify_fixture_outputs(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        seed_inputs(root)
        wording = generate_vol_targeted_growth_execution_design_approval_wording(root)
        wording_code, wording_lines = show_vol_targeted_growth_execution_design_approval_wording(root)
        record = generate_vol_targeted_growth_execution_design_approval_record(root)
        record_code, record_lines = show_vol_targeted_growth_execution_design_approval_record(root)
        if wording_code != 0:
        ...
        if summary_value(wording.summary_rows, "execution_design_approved") != "False":
            failures.append("wording report must not mark execution design approved")
        if summary_value(record.summary_rows, "execution_design_approved") != "True":
            failures.append("record report should mark design-only approval true")
        if summary_value(record.summary_rows, "execution_approved") != "False":
            failures.append("record report must keep execution_approved false")
...
def seed_inputs(root: Path) -> None:
    ...
    write_summary(
        ...
        {"closed_blocker_count": "5", "remaining_known_blockers_after_closeout": "none"},
    )
