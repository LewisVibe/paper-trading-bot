...
import trading_bot.research.qqq_leverage_validation as report
...
def main() -> int:
    ...
    if failures:
        print("QQQ leverage validation verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("QQQ leverage validation verification passed.")
    return 0
...
def verify_help_inventory(failures: list[str]) -> None:
    ...
    for command in ["--qqq-leverage-validation-report", "--show-qqq-leverage-validation-report"]:
...
def verify_static_safety(failures: list[str]) -> None:
    ...
    for token in FORBIDDEN_RUNTIME_TOKENS:
        if token in source:
            failures.append(f"QQQ leverage report references forbidden runtime token: {token}")
    early_index = bot_source.find('if sys.argv[1:] == ["--qqq-leverage-validation-report"]')
    alpaca_index = bot_source.find("from alpaca.trading.client import TradingClient")
    if early_index < 0:
        failures.append("QQQ leverage report should be handled by the early report-only route")
    elif alpaca_index >= 0 and early_index > alpaca_index:
        failures.append("QQQ leverage report route must appear before Alpaca imports")
...
def verify_deterministic_outputs(failures: list[str]) -> None:
    ...
    variants = report.build_validation_variants(
        created_at="2026-01-01T00:00:00+00:00",
        price_data=price_data,
        data_errors={},
    )
    report_rows = [report.build_report_row("2026-01-01T00:00:00+00:00", row, "full_period") for row in variants]
    cost_rows = report.build_cost_rows("2026-01-01T00:00:00+00:00", variants)
    split_rows = report.build_split_rows("2026-01-01T00:00:00+00:00", variants)
    drawdown_rows = report.build_drawdown_rows("2026-01-01T00:00:00+00:00", variants)
    summary_rows = report.build_summary_rows("2026-01-01T00:00:00+00:00", report_rows, cost_rows, split_rows)
    ...
    if {row["variant_name"] for row in variants} != expected_variants:
        failures.append("fixed QQQ validation set changed unexpectedly")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        output_paths = {name: Path(tmpdir) / path.name for name, path in report.OUTPUT_FILES.items()}
        report.write_rows(output_paths["report"], report.REPORT_COLUMNS, report_rows)
        report.write_rows(output_paths["summary"], report.SUMMARY_COLUMNS, summary_rows)
        report.write_rows(output_paths["costs"], report.COST_COLUMNS, cost_rows)
        report.write_rows(output_paths["splits"], report.SPLIT_COLUMNS, split_rows)
        report.write_rows(output_paths["drawdowns"], report.DRAWDOWN_COLUMNS, drawdown_rows)
...
def verify_labels(
    ...
    allowed = {
        ...
        "qqq_leverage_overlevered_reject",
        "qqq_leverage_cost_sensitive",
        "qqq_leverage_split_sensitive",
    ...
    if not any(row.get("variant_name") == "qqq_150_trend_gate" for row in report_rows):
        failures.append("qqq_150_trend_gate must remain part of the fixed validation set")
    if not cost_rows:
...
def verify_summary_text(
    ...
    lines = report.build_summary_lines(
        output_paths={name: path for name, path in report.OUTPUT_FILES.items()},
