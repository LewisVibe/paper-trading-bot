...
from trading_bot.research.vol_targeted_growth_gate_review import (  # noqa: E402
    FINAL_STATUS,
    OUTPUT_FILES,
    SAFETY_FLAGS,
...
COMMANDS = ["--vol-targeted-growth-gate-review", "--show-vol-targeted-growth-gate-review"]
...
TRUE_FLAGS = ["research_only", "report_only", "saved_output_only", "manual_review_only", "gate_review_only", "preview_only", "limited_manual_candidate_discussion_ready", "never_schedule_order_capable_commands"]


def main() -> int:
    failures: list[str] = []
    bot_source = read_text(ROOT / "bot.py")
    module_source = read_text(ROOT / "trading_bot" / "research" / "vol_targeted_growth_gate_review.py")
    verify_commands(bot_source, failures)
    verify_outputs_ignored(failures)
    verify_source(module_source, failures)
    verify_fixture(failures)
    if failures:
        print("Volatility-targeted growth gate review verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Volatility-targeted growth gate review verification passed.")
    return 0
...
def verify_source(source: str, failures: list[str]) -> None:
    for token in [
        FINAL_STATUS,
        "limited_manual_candidate_discussion_ready_not_approved",
        "qqq100_remains_incumbent_paper_live_seed",
        "candidate_discussion_ready_but_gate_not_enforced_and_not_approved",
        "paper_live_candidate_approved",
        "order_instructions_created",
        "execution_approved",
        "scheduling_approved",
    ...
    show_body = source_slice(source, "def show_vol_targeted_growth_gate_review", "def determine_final_status")
    if "write_rows" in show_body or "generate_vol_targeted_growth_gate_review" in show_body:
...
def verify_fixture(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        seed_inputs(root)
        result = generate_vol_targeted_growth_gate_review(root)
        if summary_value(result.summary_rows, "final_gate_review_status") != FINAL_STATUS:
            failures.append("fixture did not produce expected gate review status")
        if summary_value(result.summary_rows, "limited_manual_candidate_discussion_status") != "limited_manual_candidate_discussion_ready_not_approved":
            failures.append("fixture should allow only limited manual discussion")
        if summary_value(result.summary_rows, "qqq100_boundary_status") != "qqq100_remains_incumbent_paper_live_seed":
            failures.append("QQQ100 boundary should remain incumbent")
        for row in result.summary_rows + result.review_rows + result.evidence_rows + result.blocker_rows:
            ...
            for flag in TRUE_FLAGS:
                if str(row.get(flag, "")).lower() != "true":
                    ...
                    return
        code, lines = show_vol_targeted_growth_gate_review(root)
        if code != 0 or FINAL_STATUS not in "\n".join(lines):
            failures.append("show command did not display saved gate review")
...
def seed_inputs(root: Path) -> None:
    ...
    write_csv(data / "vol_targeted_growth_stricter_paper_live_gate_design.csv", ["gate_item"], [{"gate_item": "qqq100_incumbent_boundary"}])
    write_csv(data / "vol_targeted_growth_post_comparison_decision_summary.csv", ["summary_name", "summary_value"], [{"summary_name": "final_post_comparison_decision_status", "summary_value": "vol_targeted_growth_stricter_paper_live_discussion_gate_ready_manual_review_required"}])
