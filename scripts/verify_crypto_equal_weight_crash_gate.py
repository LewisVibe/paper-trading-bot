...
BOT = ROOT / "bot.py"
MODULE = ROOT / "trading_bot" / "research" / "crypto_equal_weight_crash_gate.py"
...
def main() -> int:
    ...
    if not MODULE.exists():
        failures.append("crypto equal-weight crash-gate module is missing")
    ...
    if failures:
        print("Crypto equal-weight crash-gate verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("Crypto equal-weight crash-gate verification passed.")
    print("Verified fixed crash-gate variants, false execution approval, transition blocking, and ignored outputs.")
    return 0
...
def verify_commands(bot_source: str, failures: list[str]) -> None:
    ...
    for command, branch in [
        ("--crypto-equal-weight-crash-gate", "if args.crypto_equal_weight_crash_gate:"),
        ("--show-crypto-equal-weight-crash-gate", "if args.show_crypto_equal_weight_crash_gate:"),
    ]:
...
def verify_module(module_source: str, failures: list[str]) -> None:
    ...
    for token in REQUIRED_TOKENS:
        if token not in module_source:
            failures.append(f"missing crash-gate token: {token}")
    for token in FORBIDDEN_TOKENS:
        if token in module_source:
            failures.append(f"forbidden execution/config token in crash-gate module: {token}")
    display_start = module_source.find("def show_crypto_equal_weight_crash_gate_file")
    if display_start == -1:
        failures.append("missing saved-display function")
    else:
        display_source = module_source[display_start:]
        if "generate_crypto_equal_weight_crash_gate" in display_source:
            failures.append("display command must not regenerate crypto crash-gate data")
...
def verify_docs(docs_source: str, failures: list[str]) -> None:
    ...
    if "catastrophic drawdown" not in docs_source:
        failures.append("docs must mention the catastrophic drawdown review purpose")
