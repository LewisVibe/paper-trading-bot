"""Verify executable-ticket draft readiness remains report-only."""
...
from trading_bot.research.vol_targeted_growth_executable_ticket_draft_readiness import (  # noqa: E402
    FINAL_DECISION,
    OUTPUT_FILES,
    generate_vol_targeted_growth_executable_ticket_draft_readiness,
    show_vol_targeted_growth_executable_ticket_draft_readiness,
)
...
def main() -> int:
    ...
    if failures:
        print("Executable-ticket draft readiness verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Executable-ticket draft readiness verification passed.")
    print("Verified readiness-only output, false order/execution flags, and no broker/order/scheduling calls.")
    return 0
...
def verify_source_safety(failures: list[str]) -> None:
    source = (ROOT / "trading_bot/research/vol_targeted_growth_executable_ticket_draft_readiness.py").read_text(encoding="utf-8")
    for phrase in [
        "TradingClient(",
        "MarketOrderRequest(",
        "submit_order",
...
def verify_fixture_outputs(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        seed_inputs(root)
        result = generate_vol_targeted_growth_executable_ticket_draft_readiness(root)
        code, lines = show_vol_targeted_growth_executable_ticket_draft_readiness(root)
        if code != 0:
        ...
        if summary_value(result.summary_rows, "final_executable_ticket_draft_readiness_decision") != FINAL_DECISION:
            failures.append("fixture should be ready for a future manual draft discussion")
        if summary_value(result.summary_rows, "draft_discussion_ready") != "True":
            failures.append("draft discussion readiness should be true when proposal quality gate passed")
