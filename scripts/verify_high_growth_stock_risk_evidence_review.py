...
BOT = ROOT / "bot.py"
MODULE = ROOT / "trading_bot" / "research" / "high_growth_stock_risk_evidence_review.py"
README = ROOT / "README.md"
CURRENT_STATE = ROOT / "docs" / "CURRENT_STATE.md"
V2_CHECKPOINT = ROOT / "docs" / "V2_RESEARCH_CHECKPOINT.md"
HERMES_TASK_BOARD = ROOT / "docs" / "HERMES_TASK_BOARD.md"

COMMANDS = ["--high-growth-stock-risk-evidence-review", "--show-high-growth-stock-risk-evidence-review"]
OUTPUTS = [
...
REQUIRED_TOKENS = [
    ...
    "evidence_supports_high_risk_research_only",
    "qqq100_clean_lead_retained",
    "broad_top1_rejected_extreme_drawdown",
    "balanced_breakout_improves_extreme_reference",
    "balanced_breakout_still_high_drawdown_vs_clean_lead",
    "cost_evidence_review_required",
    "split_evidence_review_required",
    "concentration_evidence_review_required",
...
def main() -> int:
    ...
    if failures:
        print("High-growth stock risk evidence review verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("High-growth stock risk evidence review verification passed.")
    print("Verified saved-output-only evidence review, clean lead retention, high-risk research-only label, rejected Top1 reference, ignored outputs, and non-execution boundaries.")
    return 0
...
def verify_commands(bot_source: str, failures: list[str]) -> None:
    ...
    for command, branch in [
        ("--high-growth-stock-risk-evidence-review", 'if sys.argv[1:] == ["--high-growth-stock-risk-evidence-review"]:'),
        ("--show-high-growth-stock-risk-evidence-review", 'if sys.argv[1:] == ["--show-high-growth-stock-risk-evidence-review"]:'),
    ]:
...
def verify_module(module_source: str, failures: list[str]) -> None:
    if not MODULE.exists():
        failures.append("risk evidence review module is missing")
    for output in OUTPUTS:
        if output not in module_source:
            failures.append(f"missing expected output path in module: {output}")
    for token in REQUIRED_TOKENS:
        if token not in module_source:
            failures.append(f"missing required evidence-review token: {token}")
    ...
    if "read_csv_rows" not in module_source:
        failures.append("risk evidence review should read saved CSV outputs only")
    display_start = module_source.find("def show_high_growth_stock_risk_evidence_review")
    if display_start == -1:
        failures.append("missing saved-display function")
    else:
        display_source = module_source[display_start:]
        if "generate_high_growth_stock_risk_evidence_review" in display_source:
            failures.append("display command must not regenerate the evidence review")
...
def verify_docs(docs_source: str, failures: list[str]) -> None:
    ...
    for phrase in ["high-growth stock risk evidence review", "saved-output", "research-only", "does not approve execution"]:
