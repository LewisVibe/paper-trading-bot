...
from trading_bot.research.vol_targeted_growth_post_comparison_decision import (  # noqa: E402
    OUTPUT_FILES,
    READY_STATUS,
    SAFETY_FLAGS,
...
COMMANDS = ["--vol-targeted-growth-post-comparison-decision", "--show-vol-targeted-growth-post-comparison-decision"]
...
FALSE_FLAGS = [
    ...
    "stricter_paper_live_gate_approved",
    "execution_approved",
...
TRUE_FLAGS = ["research_only", "report_only", "saved_output_only", "manual_review_only", "decision_only", "preview_only", "never_schedule_order_capable_commands"]


def main() -> int:
    failures: list[str] = []
    bot_source = read_text(ROOT / "bot.py")
    module_source = read_text(ROOT / "trading_bot" / "research" / "vol_targeted_growth_post_comparison_decision.py")
    verify_commands(bot_source, failures)
    verify_outputs_ignored(failures)
    verify_source(module_source, failures)
    verify_fixture(failures)
    if failures:
        print("Volatility-targeted growth post-comparison decision verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Volatility-targeted growth post-comparison decision verification passed.")
    return 0
...
def verify_source(source: str, failures: list[str]) -> None:
    ...
    show_body = source_slice(source, "def show_vol_targeted_growth_post_comparison_decision", "def determine_final_status")
    if "write_rows" in show_body or "generate_vol_targeted_growth_post_comparison_decision" in show_body:
...
def verify_fixture(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        seed_inputs(root)
        result = generate_vol_targeted_growth_post_comparison_decision(root)
        if summary_value(result.summary_rows, "final_post_comparison_decision_status") != READY_STATUS:
            failures.append("fixture did not produce expected ready status")
        if summary_value(result.summary_rows, "paper_live_discussion_gate_status") != "stricter_paper_live_discussion_gate_design_ready_manual_review_required":
            failures.append("fixture should move only to stricter gate design discussion")
        for row in result.summary_rows + result.decision_rows + result.evidence_rows + result.blocker_rows:
            for flag in FALSE_FLAGS:
            ...
            for flag in TRUE_FLAGS:
                if str(row.get(flag, "")).lower() != "true":
                    ...
                    return
        code, lines = show_vol_targeted_growth_post_comparison_decision(root)
        if code != 0 or READY_STATUS not in "\n".join(lines):
            failures.append("show command did not display saved decision")
...
def seed_inputs(root: Path) -> None:
    ...
    write_csv(
        ...
        ["sleeve_name", "broker_position_status"],
        [{"sleeve_name": "qqq100_core_trend_sleeve", "broker_position_status": "paper_position_present"}],
    )
    write_csv(data / "vol_targeted_growth_broker_position_comparison_blockers.csv", ["blocker_name"], [{"blocker_name": "execution_not_approved"}])
    write_csv(data / "vol_targeted_growth_paper_live_decision_summary.csv", ["summary_name", "summary_value"], [{"summary_name": "final_decision_status", "summary_value": "vol_targeted_growth_research_only_broker_comparison_discussion_ready_manual_review_required"}])
