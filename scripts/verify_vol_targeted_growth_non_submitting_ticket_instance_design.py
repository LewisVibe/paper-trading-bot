...
from trading_bot.research.vol_targeted_growth_non_submitting_ticket_instance_design import (  # noqa: E402
    FINAL_DECISION,
    FINAL_STATUS,
    OUTPUT_FILES,
...
def main() -> int:
    ...
    if failures:
        print("Volatility-targeted non-submitting ticket-instance design verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("Volatility-targeted non-submitting ticket-instance design verification passed.")
    print("Verified design-only output, no executable ticket, blank order values, false approvals, and no broker/order/scheduling calls.")
    return 0
...
def verify_source_boundaries(failures: list[str]) -> None:
    source = read_text(ROOT / "trading_bot" / "research" / "vol_targeted_growth_non_submitting_ticket_instance_design.py")
    for token in FORBIDDEN_SOURCE_TOKENS:
        if token in source:
            failures.append(f"ticket-instance design source contains forbidden token: {token}")
    for phrase in [
        "ticket_instance_design_only",
        "non_submitting",
        "NON_SUBMITTING_TICKET_INSTANCE_DESIGNED_NO_ORDER_VALUES",
        "ticket_instance_created",
        "executable_ticket_created",
        "order_values_populated",
        "orders_submitted",
        ...
        if phrase not in source:
            failures.append(f"ticket-instance design source missing safety phrase: {phrase}")
...
def verify_vps_daily_summary_integration(failures: list[str]) -> None:
    ...
    for phrase in [
        "VOL_NON_SUBMITTING_TICKET_INSTANCE_DESIGN_SUMMARY_PATH",
        "Volatility non-submitting ticket-instance design:",
        "vol_non_submitting_ticket_instance_design_status_lines",
        "NON_SUBMITTING_TICKET_INSTANCE_DESIGNED_NO_ORDER_VALUES",
        "vol_non_submitting_ticket_instance_design_warning: monitor only;",
    ]:
        if phrase not in source:
            failures.append(f"VPS daily summary missing non-submitting ticket-instance phrase: {phrase}")
...
def verify_fixture_output(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        seed_inputs(root)
        result = generate_vol_targeted_growth_non_submitting_ticket_instance_design(root)
        code, lines = show_vol_targeted_growth_non_submitting_ticket_instance_design(root)
        if code != 0:
...
def verify_summary_rows(rows: list[dict[str, object]], failures: list[str]) -> None:
    if summary_value(rows, "final_ticket_instance_design_status") != FINAL_STATUS:
        failures.append("summary final status is incorrect")
    if summary_value(rows, "final_ticket_instance_design_decision") != FINAL_DECISION:
    ...
    if summary_value(rows, "executable_ticket_created") != "False":
        failures.append("summary must not create an executable ticket")
...
def verify_ticket_rows(rows: list[dict[str, object]], failures: list[str]) -> None:
    field_names = {str(row.get("ticket_field", "")) for row in rows}
    missing = ORDER_VALUE_FIELDS - field_names
    if missing:
        failures.append(f"ticket rows missing protected fields: {sorted(missing)}")
    for row in rows:
        field = str(row.get("ticket_field", ""))
        if field in ORDER_VALUE_FIELDS and str(row.get("draft_value", "")).strip():
            failures.append(f"ticket field must not have a draft order value: {field}")
        for flag in FALSE_FLAGS:
            if str(row.get(flag, "")).strip() != "False":
                failures.append(f"ticket flag must be False for {field}: {flag}")
