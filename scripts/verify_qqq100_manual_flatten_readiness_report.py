...
BOT = ROOT / "bot.py"
MODULE = ROOT / "trading_bot" / "research" / "qqq100_manual_flatten_readiness_report.py"
...
COMMANDS = ["--qqq100-manual-flatten-readiness-report", "--show-qqq100-manual-flatten-readiness-report"]
OUTPUTS = [
...
REQUIRED_MODULE_TOKENS = [
    "flatten_not_needed_currently",
    "future_manual_flatten_discussion_possible_not_approved",
    "flatten_not_needed_already_flat",
    "blocked_manual_review_required",
    "manual_flatten_not_approved_by_readiness_report",
...
def main() -> int:
    ...
    if failures:
        print("QQQ100 manual flatten readiness report verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("QQQ100 manual flatten readiness report verification passed.")
    print("Verified saved-output flatten readiness, current no-action state, future flat-plus-one manual-review state, false approvals, ignored outputs, and no broker/order/config/scheduling calls.")
    return 0
...
def verify_commands(bot_source: str, failures: list[str]) -> None:
    ...
    branches = {
        "--qqq100-manual-flatten-readiness-report": 'if sys.argv[1:] == ["--qqq100-manual-flatten-readiness-report"]:',
        "--show-qqq100-manual-flatten-readiness-report": 'if sys.argv[1:] == ["--show-qqq100-manual-flatten-readiness-report"]:',
    }
...
def verify_module(module_source: str, failures: list[str]) -> None:
    if not MODULE.exists():
        failures.append("QQQ100 manual flatten readiness module is missing")
    ...
    if "evaluate_paper_live_saved_evidence" not in module_source:
        failures.append("flatten readiness should use saved paper-live evidence helper")
    if "evaluate_followup_policy" not in module_source:
        failures.append("flatten readiness should use saved follow-up policy helper")
    display_start = module_source.find("def show_qqq100_manual_flatten_readiness_report")
    if display_start == -1:
        failures.append("missing saved-display function")
    elif "generate_qqq100_manual_flatten_readiness_report" in module_source[display_start:]:
...
def verify_fixture_output(failures: list[str]) -> None:
    from trading_bot.research.qqq100_manual_flatten_readiness_report import (  # noqa: PLC0415
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = generate_qqq100_manual_flatten_readiness_report(root)
        status_code, lines = show_qqq100_manual_flatten_readiness_report(root)
    ...
    expected_flat = {
        ...
        "saved_position_state": "paper_position_long",
        "saved_position_quantity": "1",
        "followup_policy_status": "future_manual_flatten_discussion_possible",
        "manual_flatten_discussion_status": "manual_flatten_discussion_possible_after_separate_approval",
        "largest_blocker": "manual_flatten_not_approved_by_readiness_report",
        "recommended_next_step": "separate_manual_flatten_readiness_review_required_before_any_action",
    }
