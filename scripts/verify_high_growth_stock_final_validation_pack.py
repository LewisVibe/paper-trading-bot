...
BOT = ROOT / "bot.py"
MODULE = ROOT / "trading_bot" / "research" / "high_growth_stock_final_validation_pack.py"
README = ROOT / "README.md"
CURRENT_STATE = ROOT / "docs" / "CURRENT_STATE.md"
V2_CHECKPOINT = ROOT / "docs" / "V2_RESEARCH_CHECKPOINT.md"
HERMES_TASK_BOARD = ROOT / "docs" / "HERMES_TASK_BOARD.md"

COMMANDS = ["--high-growth-stock-final-validation-pack", "--show-high-growth-stock-final-validation-pack"]
OUTPUTS = [
...
REQUIRED_TOKENS = [
    "high_growth_final_validation_required",
    ...
    "ready_for_manual_preview_candidate_discussion",
    "needs_more_research_validation",
    "paused_due_to_drawdown_or_evidence_quality",
    "insufficient_saved_evidence",
    "qqq100_clean_lead_retained",
    "broad_top1_rejected_extreme_drawdown",
    "high_risk_candidate_research_only",
...
def main() -> int:
    ...
    if failures:
        print("High-growth stock final validation pack verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("High-growth stock final validation pack verification passed.")
    print("Verified saved-output-only final validation, clean lead retention, high-risk research-only boundaries, rejected Top1 reference, ignored outputs, and blocked preview/execution.")
    return 0
...
def verify_commands(bot_source: str, failures: list[str]) -> None:
    ...
    for command, branch in [
        ("--high-growth-stock-final-validation-pack", 'if sys.argv[1:] == ["--high-growth-stock-final-validation-pack"]:'),
        ("--show-high-growth-stock-final-validation-pack", 'if sys.argv[1:] == ["--show-high-growth-stock-final-validation-pack"]:'),
    ]:
...
def verify_module(module_source: str, failures: list[str]) -> None:
    if not MODULE.exists():
        failures.append("final validation pack module is missing")
    for output in OUTPUTS:
        if output not in module_source:
            failures.append(f"missing expected output path in module: {output}")
    for token in REQUIRED_TOKENS:
        if token not in module_source:
            failures.append(f"missing required final-validation token: {token}")
    ...
    if "read_csv_rows" not in module_source:
        failures.append("final validation pack should read saved CSV outputs only")
    display_start = module_source.find("def show_high_growth_stock_final_validation_pack")
    if display_start == -1:
        failures.append("missing saved-display function")
    else:
        display_source = module_source[display_start:]
        if "generate_high_growth_stock_final_validation_pack" in display_source:
            failures.append("display command must not regenerate the final validation pack")
...
def verify_docs(docs_source: str, failures: list[str]) -> None:
    ...
    for phrase in ["high-growth stock final validation pack", "saved-output", "research-only", "does not approve execution"]:
