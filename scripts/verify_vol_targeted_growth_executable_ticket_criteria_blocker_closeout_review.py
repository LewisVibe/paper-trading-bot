...
BOT = ROOT / "bot.py"
MODULE = ROOT / "trading_bot" / "research" / "vol_targeted_growth_executable_ticket_criteria_blocker_closeout_review.py"
...
OUTPUTS = [
    "data/vol_targeted_growth_executable_ticket_criteria_blocker_closeout_review.csv",
    "data/vol_targeted_growth_executable_ticket_criteria_blocker_closeout_review_summary.csv",
    "data/vol_targeted_growth_executable_ticket_criteria_blocker_closeout_review_blockers.csv",
    "data/vol_targeted_growth_executable_ticket_criteria_blocker_closeout_review_evidence.csv",
]

REQUIRED_TOKENS = [
    "vol_targeted_growth_executable_ticket_criteria_blocker_closeout_review_manual_review_required",
    "CRITERIA_BLOCKERS_REVIEWED_NONE_CLOSED",
    "manual_review_each_criteria_blocker_before_any_closeout",
...
def main() -> int:
    ...
    if failures:
        print("Volatility-targeted executable-ticket criteria blocker closeout review verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("Volatility-targeted executable-ticket criteria blocker closeout review verification passed.")
    print("Verified closeout-review-only output, no blocker closure, false approvals, ignored outputs, and dashboard integration.")
    return 0
...
def verify_module(module_source: str, failures: list[str]) -> None:
    if not MODULE.exists():
        failures.append("criteria blocker closeout review module is missing")
...
def verify_fixture_output(failures: list[str]) -> None:
    ...
    from trading_bot.research.vol_targeted_growth_executable_ticket_criteria_blocker_closeout_review import (  # noqa: PLC0415
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        result = generate_vol_targeted_growth_executable_ticket_criteria_blocker_closeout_review(root)
        status_code, lines = show_vol_targeted_growth_executable_ticket_criteria_blocker_closeout_review(root)

    output = "\n".join(result.summary_lines + lines)
    if status_code != 0:
        failures.append("criteria blocker closeout review display should return 0 after generation")
    for phrase in [
        "vol_targeted_growth_executable_ticket_criteria_blocker_closeout_review_manual_review_required",
        "CRITERIA_BLOCKERS_REVIEWED_NONE_CLOSED",
...
def verify_dashboard_integration(failures: list[str]) -> None:
    ...
    for phrase in [
        "vol_ticket_criteria_blocker_closeout_review",
        "vol_targeted_growth_executable_ticket_criteria_blocker_closeout_review_summary.csv",
        ...
        if phrase not in source:
            failures.append(f"go/no-go dashboard missing blocker closeout review integration phrase: {phrase}")
