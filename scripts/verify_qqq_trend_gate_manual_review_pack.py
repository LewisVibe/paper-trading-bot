...
BOT = ROOT / "bot.py"
MODULE = ROOT / "trading_bot" / "research" / "qqq_trend_gate_manual_review.py"
...
REQUIRED_TOKENS = [
    ...
    "Monday paper smoke test remains a connectivity/order-path test only",
    '"research_only": True',
    '"preview_only": False',
...
def main() -> int:
    ...
    if not MODULE.exists():
        failures.append("QQQ trend-gate manual review module is missing")
    ...
    if failures:
        print("QQQ trend-gate manual review pack verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("QQQ trend-gate manual review pack verification passed.")
    print("Verified saved-output QQQ manual review pack, false approval flags, non-execution wording, and ignored outputs.")
    return 0
...
def verify_commands(bot_source: str, failures: list[str]) -> None:
    ...
    for command, branch in [
        ("--qqq-trend-gate-manual-review-pack", 'if sys.argv[1:] == ["--qqq-trend-gate-manual-review-pack"]:'),
        ("--show-qqq-trend-gate-manual-review-pack", 'if sys.argv[1:] == ["--show-qqq-trend-gate-manual-review-pack"]:'),
    ]:
...
def verify_module(module_source: str, failures: list[str]) -> None:
    ...
    for token in REQUIRED_TOKENS:
        if token not in module_source:
            failures.append(f"missing QQQ manual-review token: {token}")
    for token in FORBIDDEN_TOKENS:
        if token in module_source:
            failures.append(f"forbidden execution/config/scheduling token in module: {token}")
    display_start = module_source.find("def show_qqq_trend_gate_manual_review_pack")
    if display_start == -1:
        failures.append("missing saved-display function")
    else:
        display_source = module_source[display_start:]
        if "generate_qqq_trend_gate_manual_review_pack" in display_source:
