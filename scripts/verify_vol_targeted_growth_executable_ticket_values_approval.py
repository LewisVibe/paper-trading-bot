"""Verify executable ticket-values approval checkpoints remain non-executing."""
...
from trading_bot.research.vol_targeted_growth_executable_ticket_values_approval import (  # noqa: E402
    RECORD_DECISION,
    RECORD_OUTPUTS,
    WORDING_DECISION,
    WORDING_OUTPUTS,
    generate_vol_targeted_growth_executable_ticket_values_approval_record,
    generate_vol_targeted_growth_executable_ticket_values_approval_wording,
    show_vol_targeted_growth_executable_ticket_values_approval_record,
)


COMMANDS = [
    "--vol-targeted-growth-executable-ticket-values-approval-wording",
    "--show-vol-targeted-growth-executable-ticket-values-approval-wording",
    "--vol-targeted-growth-executable-ticket-values-approval-record",
    "--show-vol-targeted-growth-executable-ticket-values-approval-record",
]
...
def main() -> int:
    ...
    if failures:
        print("Executable ticket-values approval verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Executable ticket-values approval verification passed.")
    print("Verified explicit approval wording/record, false order/execution flags, and no broker/order/scheduling calls.")
    return 0
...
def verify_source_safety(failures: list[str]) -> None:
    source = (ROOT / "trading_bot/research/vol_targeted_growth_executable_ticket_values_approval.py").read_text(encoding="utf-8")
    for phrase in [
        "TradingClient(",
        "MarketOrderRequest(",
        "submit_order",
...
def verify_fixture_outputs(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        seed_inputs(root)
        wording = generate_vol_targeted_growth_executable_ticket_values_approval_wording(root)
        record = generate_vol_targeted_growth_executable_ticket_values_approval_record(root)
        code, lines = show_vol_targeted_growth_executable_ticket_values_approval_record(root)
        if code != 0:
        ...
        for phrase in [
            ...
            "executable_ticket_values_approval_requested=True",
