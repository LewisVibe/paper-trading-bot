...
BOT = ROOT / "bot.py"
MODULE = ROOT / "trading_bot" / "research" / "paper_order_smoke_test_postcheck.py"
COMMAND = "--paper-order-smoke-test-postcheck"
OUTPUT = "data/paper_order_smoke_test_postcheck.csv"

REQUIRED_TOKENS = [
    OUTPUT,
    "postcheck_requires_confirmed_readonly_check",
    "postcheck_order_observed_filled_manual_review",
    "postcheck_order_observed_open_or_queued",
    "postcheck_order_observed_not_filled_manual_review",
    "postcheck_no_matching_order_found",
...
def main() -> int:
    ...
    if failures:
        print("Paper-order smoke-test postcheck verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("Paper-order smoke-test postcheck verification passed.")
...
def verify_command_registered(failures: list[str]) -> None:
    ...
    if early_route_index == -1:
        failures.append("missing early route for paper-order smoke-test postcheck")
    elif alpaca_import_index != -1 and early_route_index > alpaca_import_index:
        failures.append("postcheck early route should be before broker imports")
    branch_index = source.find("if args.paper_order_smoke_test_postcheck:")
    load_config_index = source.find("config = load_config(")
    if branch_index == -1:
        failures.append("missing argparse branch for postcheck")
    elif load_config_index != -1 and branch_index > load_config_index:
        failures.append("postcheck must route before normal config loading")
...
def verify_module_source(failures: list[str]) -> None:
    ...
    for token in REQUIRED_TOKENS:
        if token not in source:
            failures.append(f"missing postcheck token: {token}")
    ...
    for sensitive in ["account_id", "account_number", "api_key=", "secret_key=", "order.id"]:
        if sensitive in source:
            failures.append(f"source should avoid sensitive/full identifier token: {sensitive}")
...
def verify_default_mode_no_alpaca(failures: list[str]) -> None:
    with TemporaryDirectory() as tmp:
        ...
        code = (
            ...
            "r=generate_paper_order_smoke_test_postcheck('AAPL', 'buy', '1', False, r'.'); "
        ...
        for sensitive in ["account_id", "account_number", "api_key", "secret_key"]:
            if sensitive in completed.stdout.lower():
                failures.append(f"terminal summary should not print sensitive token: {sensitive}")
        output = root / OUTPUT
        if not output.exists():
            failures.append("default fixture run did not write expected postcheck CSV")
            return
        with output.open(newline="", encoding="utf-8") as handle:
            rows = list(csv.DictReader(handle))
        if not rows:
            failures.append("postcheck wrote no rows")
            return
        for column in ["order_execution_approved", "execution_approved", "scheduling_approved", "followup_order_approved"]:
        ...
        final = next((row for row in rows if row.get("check_name") == "final_postcheck_status"), {})
        if final.get("check_status") != "postcheck_requires_confirmed_readonly_check":
            failures.append(f"default final status should require confirmed read-only check, got {final.get('check_status')}")
...
def write_fixture(root: Path) -> None:
    ...
    write_csv(
        root / "data" / "paper_order_smoke_test_live_preflight.csv",
        ["check_name", "check_status"],
        [{"check_name": "final_live_preflight_status", "check_status": "live_preflight_wait_for_market_open"}],
    )
    write_csv(
