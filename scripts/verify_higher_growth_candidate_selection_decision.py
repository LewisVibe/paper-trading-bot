...
from trading_bot.research.higher_growth_candidate_selection_decision import (  # noqa: E402
    BALANCED_CANDIDATE,
...
EXPECTED_OUTPUTS = [
    "data/higher_growth_candidate_selection_decision.csv",
    "data/higher_growth_candidate_selection_summary.csv",
    "data/higher_growth_candidate_selection_evidence.csv",
    "data/higher_growth_candidate_selection_blockers.csv",
]
...
def main() -> int:
    ...
    module_source = read_text(ROOT / "trading_bot" / "research" / "higher_growth_candidate_selection_decision.py")
    ...
    if failures:
        print("Higher-growth candidate selection decision verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("Higher-growth candidate selection decision verification passed.")
    print("Verified selected candidate, balanced runner-up, crypto deferral, false approvals, and no broker/order/config/scheduling paths.")
    return 0
...
def verify_command_registration(bot_source: str, failures: list[str]) -> None:
    ...
    for token in ["generate_higher_growth_candidate_selection_decision", "show_higher_growth_candidate_selection_decision"]:
...
def verify_source_boundaries(module_source: str, failures: list[str]) -> None:
    required = [
        ...
        "design_saved_output_preview_only_mode_for_higher_growth_70_20_5_5",
        "preview_candidate_approved",
        "preview_implementation_approved",
        "execution_approved",
        "paper_execution_approved",
    ...
    show_body = source_slice(module_source, "def show_higher_growth_candidate_selection_decision", "def build_decision_rows")
    if "write_rows" in show_body or "generate_higher_growth_candidate_selection_decision" in show_body:
...
def verify_fixture_generation(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write_csv(
            ...
            [{"summary_name": "final_readiness_status", "summary_value": "higher_growth_preview_discussion_ready_manual_review_required"}],
        )
        write_csv(data / "higher_growth_preview_readiness_pack.csv", ["check_name", "status"], [{"check_name": "preview_boundary", "status": "preview_implementation_not_added"}])
        write_csv(data / "high_growth_strategy_discovery_sprint.csv", ["candidate_name", "final_candidate_status"], [{"candidate_name": SELECTED_CANDIDATE, "final_candidate_status": "strong_high_growth_candidate_research_only"}])
        write_csv(
            data / "multi_sleeve_higher_growth_review.csv",
            ["allocation_name", "CAGR", "Sharpe", "MaxDD", "Calmar"],
            [{"allocation_name": SELECTED_CANDIDATE, "CAGR": "23.6634", "Sharpe": "1.2232", "MaxDD": "-22.5209", "Calmar": "1.0507"}],
        )
        write_csv(
        ...
        write_csv(
            ...
            [
                ...
                {"portfolio_name": BALANCED_CANDIDATE, "candidate_cagr": "20.9941", "candidate_sharpe": "1.1947", "candidate_max_drawdown": "-21.6286", "candidate_calmar": "0.9707"},
                {"portfolio_name": CRYPTO_BLEND_CANDIDATE, "candidate_cagr": "21.7328", "candidate_sharpe": "1.1852", "candidate_max_drawdown": "-22.2489", "candidate_calmar": "0.9768"},
            ],
        )

        result = generate_higher_growth_candidate_selection_decision(root)
        status = summary_value(result.summary_rows, "final_selection_status")
        if status != FINAL_STATUS:
            failures.append(f"fixture should produce selected status, got {status}")
        selected = summary_value(result.summary_rows, "selected_candidate")
        if selected != SELECTED_CANDIDATE:
            failures.append(f"selected candidate should be {SELECTED_CANDIDATE}, got {selected}")
        if "not_selected_crypto_policy_review_required" not in "\n".join(str(row) for row in result.decision_rows + result.summary_rows):
            failures.append("crypto blend should be deferred for crypto policy review")
            ...
            for row in collection:
                ...
                for flag in TRUE_FLAGS:
                    if str(row.get(flag, "")).lower() != "true":
                        ...
                        return
        code, lines = show_higher_growth_candidate_selection_decision(root)
        display = "\n".join(lines)
        if code != 0:
            failures.append(f"show command should succeed after generation, got {code}")
        for token in [FINAL_STATUS, SELECTED_CANDIDATE, "execution_approved=false", "preview_candidate_approved=false"]:
