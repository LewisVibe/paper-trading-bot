...
BOT = ROOT / "bot.py"
MODULE = ROOT / "trading_bot" / "research" / "vol_targeted_growth_executable_ticket_criteria_resolution_plan.py"
...
OUTPUTS = [
    "data/vol_targeted_growth_executable_ticket_criteria_resolution_plan.csv",
    "data/vol_targeted_growth_executable_ticket_criteria_resolution_plan_summary.csv",
    "data/vol_targeted_growth_executable_ticket_criteria_resolution_plan_blockers.csv",
    "data/vol_targeted_growth_executable_ticket_criteria_resolution_plan_evidence.csv",
]

REQUIRED_TOKENS = [
    "vol_targeted_growth_executable_ticket_criteria_resolution_plan_created_manual_review_required",
    "CRITERIA_BLOCKER_RESOLUTION_PLAN_CREATED_APPROVAL_STILL_BLOCKED",
    "manual_review_resolution_plan_before_any_approval_readiness_change",
...
def main() -> int:
    ...
    if failures:
        print("Volatility-targeted executable-ticket criteria resolution plan verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("Volatility-targeted executable-ticket criteria resolution plan verification passed.")
    print("Verified plan-only output, unresolved blockers, false approvals, ignored outputs, and dashboard integration.")
    return 0
...
def verify_module(module_source: str, failures: list[str]) -> None:
    if not MODULE.exists():
        failures.append("criteria resolution plan module is missing")
...
def verify_fixture_output(failures: list[str]) -> None:
    ...
    from trading_bot.research.vol_targeted_growth_executable_ticket_criteria_resolution_plan import (  # noqa: PLC0415
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write_blockers(data / "vol_targeted_growth_executable_ticket_approval_criteria_blockers.csv")
        ...
        result = generate_vol_targeted_growth_executable_ticket_criteria_resolution_plan(root)
        status_code, lines = show_vol_targeted_growth_executable_ticket_criteria_resolution_plan(root)

    output = "\n".join(result.summary_lines + lines)
    if status_code != 0:
        failures.append("criteria resolution plan display should return 0 after generation")
    for phrase in [
        "vol_targeted_growth_executable_ticket_criteria_resolution_plan_created_manual_review_required",
        "CRITERIA_BLOCKER_RESOLUTION_PLAN_CREATED_APPROVAL_STILL_BLOCKED",
...
def verify_dashboard_integration(failures: list[str]) -> None:
    ...
    for phrase in [
        "vol_ticket_criteria_resolution",
        "vol_targeted_growth_executable_ticket_criteria_resolution_plan_summary.csv",
        ...
        if phrase not in source:
            failures.append(f"go/no-go dashboard missing criteria resolution integration phrase: {phrase}")
...
def write_blockers(path: Path) -> None:
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=["blocker_name", "status"])
        writer.writeheader()
        for name in ["approval_not_requested", "approval_readiness_not_ready", "prerequisites_not_closed"]:
            writer.writerow({"blocker_name": name, "status": "blocked"})
