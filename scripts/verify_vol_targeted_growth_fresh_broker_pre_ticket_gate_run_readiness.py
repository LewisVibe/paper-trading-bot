...
from trading_bot.research.vol_targeted_growth_fresh_broker_pre_ticket_gate_run_readiness import (  # noqa: E402
...
def main() -> int:
    ...
    if failures:
        print("Volatility-targeted fresh broker pre-ticket gate run-readiness verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("Volatility-targeted fresh broker pre-ticket gate run-readiness verification passed.")
    print("Verified readiness-only output, no Alpaca call, no broker read, no run approval, no order values, false approvals, and daily summary integration.")
    return 0
...
def verify_source_boundaries(failures: list[str]) -> None:
    source = read_text(ROOT / "trading_bot" / "research" / "vol_targeted_growth_fresh_broker_pre_ticket_gate_run_readiness.py")
    for token in FORBIDDEN_SOURCE_TOKENS:
        if token in source:
            failures.append(f"run-readiness source contains forbidden token: {token}")
    for phrase in [
        "run_readiness_only",
        ...
        if phrase not in source:
            failures.append(f"run-readiness source missing safety phrase: {phrase}")
...
def verify_vps_daily_summary_integration(failures: list[str]) -> None:
    ...
    for phrase in [
        "VOL_FRESH_BROKER_PRE_TICKET_GATE_RUN_READINESS_SUMMARY_PATH",
        "Volatility fresh broker pre-ticket gate run-readiness:",
        "vol_fresh_broker_pre_ticket_gate_run_readiness_status_lines",
        "READY_TO_REQUEST_EXPLICIT_READONLY_ALPACA_APPROVAL",
        "vol_fresh_broker_pre_ticket_gate_run_readiness_warning: monitor only;",
    ]:
        if phrase not in source:
            failures.append(f"VPS daily summary missing fresh broker pre-ticket run-readiness phrase: {phrase}")
...
def verify_fixture_output(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        seed_inputs(root)
        result = generate_vol_targeted_growth_fresh_broker_pre_ticket_gate_run_readiness(root)
        code, lines = show_vol_targeted_growth_fresh_broker_pre_ticket_gate_run_readiness(root)
        if code != 0:
        ...
        for phrase in [
            FINAL_STATUS_READY,
            FINAL_DECISION_READY,
            "ready_to_request_readonly_approval=True",
...
def verify_summary_rows(rows: list[dict[str, object]], failures: list[str]) -> None:
    if summary_value(rows, "final_pre_ticket_gate_run_readiness_status") != FINAL_STATUS_READY:
        failures.append("summary final status is incorrect")
    if summary_value(rows, "final_pre_ticket_gate_run_readiness_decision") != FINAL_DECISION_READY:
        failures.append("summary final decision is incorrect")
    if summary_value(rows, "ready_to_request_readonly_approval") != "True":
        failures.append("summary should be ready to request future read-only approval with complete fixture inputs")
...
def verify_check_rows(rows: list[dict[str, object]], failures: list[str]) -> None:
    required = {
        "gate_design_final_pre_ticket_gate_design_decision",
        "future_prompt_must_explicitly_confirm_readonly_alpaca",
        "gate_run_must_not_create_orders",
        "gate_run_must_not_populate_order_values",
        "gate_run_must_not_approve_execution_or_scheduling",
    }
    names = {str(row.get("readiness_check", "")) for row in rows}
    missing = required - names
    if missing:
        failures.append(f"readiness checks missing required rows: {sorted(missing)}")
    for row in rows:
        if str(row.get("status", "")).strip() != "pass":
            failures.append(f"fixture readiness check should pass: {row.get('readiness_check')}")
        for flag in FALSE_FLAGS:
            if str(row.get(flag, "")).strip() != "False":
                failures.append(f"readiness check flag must be False for {row.get('readiness_check')}: {flag}")
