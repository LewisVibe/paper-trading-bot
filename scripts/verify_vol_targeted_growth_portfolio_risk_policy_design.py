...
from trading_bot.research.vol_targeted_growth_portfolio_risk_policy_design import (  # noqa: E402
    FINAL_STATUS,
    OUTPUT_FILES,
    SAFETY_FLAGS,
...
COMMANDS = ["--vol-targeted-growth-portfolio-risk-policy-design", "--show-vol-targeted-growth-portfolio-risk-policy-design"]
...
TRUE_FLAGS = ["research_only", "report_only", "saved_output_only", "manual_review_only", "policy_design_only", "never_schedule_order_capable_commands"]


def main() -> int:
    failures: list[str] = []
    bot_source = read_text(ROOT / "bot.py")
    module_source = read_text(ROOT / "trading_bot" / "research" / "vol_targeted_growth_portfolio_risk_policy_design.py")
    verify_commands(bot_source, failures)
    verify_outputs_ignored(failures)
    verify_source(module_source, failures)
    verify_fixture(failures)
    if failures:
        print("Volatility-targeted growth portfolio-risk policy design verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Volatility-targeted growth portfolio-risk policy design verification passed.")
    return 0
...
def verify_source(source: str, failures: list[str]) -> None:
    for token in [
        FINAL_STATUS,
        "policy_not_enforced_design_only",
        "paper_live_discussion_not_approved_research_only",
        "target_weight=20%",
        "target_weight=5%",
        "drawdown_guardrail",
        "broker_position_guardrail",
        "execution_approved",
        "scheduling_approved",
    ]:
    ...
    show_body = source_slice(source, "def show_vol_targeted_growth_portfolio_risk_policy_design", "def build_policy_rows")
    if "write_rows" in show_body or "generate_vol_targeted_growth_portfolio_risk_policy_design" in show_body:
...
def verify_fixture(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        data = root / "data"
        data.mkdir()
        write_csv(data / "vol_targeted_growth_portfolio_risk_review_summary.csv", ["summary_name", "summary_value"], [{"summary_name": "final_risk_review_status", "summary_value": "vol_targeted_growth_portfolio_risk_manual_review_required"}])
        result = generate_vol_targeted_growth_portfolio_risk_policy_design(root)
        if summary_value(result.summary_rows, "final_policy_design_status") != FINAL_STATUS:
            failures.append("fixture did not produce expected policy design status")
        if summary_value(result.summary_rows, "policy_enforcement_status") != "policy_not_enforced_design_only":
            failures.append("policy must remain unenforced")
        if summary_value(result.summary_rows, "paper_live_discussion_status") != "paper_live_discussion_not_approved_research_only":
            failures.append("paper-live discussion should remain not approved")
        if len(result.policy_rows) < 8:
            failures.append("expected policy design guardrail rows")
        for row in result.summary_rows + result.policy_rows + result.evidence_rows + result.blocker_rows:
            for flag in FALSE_FLAGS:
            ...
            for flag in TRUE_FLAGS:
                if str(row.get(flag, "")).lower() != "true":
                    ...
                    return
        code, lines = show_vol_targeted_growth_portfolio_risk_policy_design(root)
        if code != 0 or FINAL_STATUS not in "\n".join(lines):
            failures.append("show command did not display saved policy design")
