"""Verify saved-price snapshot runner implementation readiness is report-only."""
...
from trading_bot.research.vol_targeted_growth_saved_price_snapshot_runner_readiness import (  # noqa: E402
    OUTPUT_FILES,
    READY_DECISION,
...
FALSE_FLAGS = [
    "runner_implementation_approved",
...
def main() -> int:
    ...
    if failures:
        print("Saved-price snapshot runner readiness verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Saved-price snapshot runner readiness verification passed.")
    print("Verified implementation discussion readiness only, no runner implementation, no price fetches, false approvals, and no broker/order/scheduling calls.")
    return 0
...
def verify_source_safety(failures: list[str]) -> None:
    source = (ROOT / "trading_bot/research/vol_targeted_growth_saved_price_snapshot_runner_readiness.py").read_text(encoding="utf-8")
    for token in [
        "TradingClient(",
        "MarketOrderRequest(",
        "submit_order",
...
def verify_fixture_output(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        seed_inputs(root)
        result = generate_vol_targeted_growth_saved_price_snapshot_runner_readiness(root)
        code, lines = show_vol_targeted_growth_saved_price_snapshot_runner_readiness(root)
        if code != 0:
        ...
        if summary_value(result.summary_rows, "runner_implementation_discussion_ready") != "True":
            failures.append("fixture should be ready to discuss runner implementation")
        if summary_value(result.summary_rows, "runner_implementation_approved") != "False":
            failures.append("readiness must not approve implementation")
