"""Verify review-only draft ticket values remain non-executable."""
...
from trading_bot.research.vol_targeted_growth_review_only_draft_ticket_values import (  # noqa: E402
    DRAFT_DECISION,
    DRAFT_OUTPUTS,
    QUALITY_DECISION,
    QUALITY_OUTPUTS,
    generate_vol_targeted_growth_review_only_draft_ticket_values,
    generate_vol_targeted_growth_review_only_draft_ticket_values_quality_gate,
    show_vol_targeted_growth_review_only_draft_ticket_values_quality_gate,
)


COMMANDS = [
    "--vol-targeted-growth-review-only-draft-ticket-values",
    "--show-vol-targeted-growth-review-only-draft-ticket-values",
    "--vol-targeted-growth-review-only-draft-ticket-values-quality-gate",
    "--show-vol-targeted-growth-review-only-draft-ticket-values-quality-gate",
]
...
def main() -> int:
    ...
    if failures:
        print("Review-only draft ticket values verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Review-only draft ticket values verification passed.")
    print("Verified review-only draft values, quality gate, false order/execution flags, and no broker/order/scheduling calls.")
    return 0
...
def verify_source_safety(failures: list[str]) -> None:
    source = (ROOT / "trading_bot/research/vol_targeted_growth_review_only_draft_ticket_values.py").read_text(encoding="utf-8")
    for phrase in [
        "TradingClient(",
        "MarketOrderRequest(",
        "submit_order",
...
def verify_fixture_outputs(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        seed_inputs(root)
        draft = generate_vol_targeted_growth_review_only_draft_ticket_values(root)
        quality = generate_vol_targeted_growth_review_only_draft_ticket_values_quality_gate(root)
        code, lines = show_vol_targeted_growth_review_only_draft_ticket_values_quality_gate(root)
        if code != 0:
        ...
        if summary_value(quality.summary_rows, "quality_gate_passed") != "True":
            failures.append("quality gate should pass for review-only draft ticket values")
        verify_false_flags(quality.summary_rows, failures)
        verify_value_rows(draft.value_rows, failures)
...
def verify_value_rows(rows: list[dict[str, object]], failures: list[str]) -> None:
    fields = {str(row.get("draft_value_name", "")): str(row.get("draft_value", "")).strip().lower() for row in rows}
    for required in ["draft_sleeve_targets", "draft_side_label", "draft_quantity_label", "draft_order_type_label", "draft_time_in_force_label"]:
        if required not in fields:
            failures.append(f"missing draft value field: {required}")
