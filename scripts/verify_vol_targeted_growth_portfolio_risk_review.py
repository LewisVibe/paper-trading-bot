...
from trading_bot.research.vol_targeted_growth_portfolio_risk_review import (  # noqa: E402
    FINAL_STATUS,
    OUTPUT_FILES,
    SAFETY_FLAGS,
...
COMMANDS = ["--vol-targeted-growth-portfolio-risk-review", "--show-vol-targeted-growth-portfolio-risk-review"]
...
def main() -> int:
    ...
    module_source = read_text(ROOT / "trading_bot" / "research" / "vol_targeted_growth_portfolio_risk_review.py")
    verify_commands(bot_source, failures)
    verify_outputs_ignored(failures)
    verify_source(module_source, failures)
    verify_fixture(failures)
    if failures:
        print("Volatility-targeted growth portfolio-risk review verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Volatility-targeted growth portfolio-risk review verification passed.")
    return 0
...
def verify_source(source: str, failures: list[str]) -> None:
    for token in [FINAL_STATUS, "paper_live_discussion_not_approved_research_only", "broker_position_comparison_and_portfolio_risk_policy_not_reviewed", "execution_approved", "scheduling_approved"]:
    ...
    show_body = source_slice(source, "def show_vol_targeted_growth_portfolio_risk_review", "def build_review_rows")
    if "write_rows" in show_body or "generate_vol_targeted_growth_portfolio_risk_review" in show_body:
...
def verify_fixture(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write_csv(data / "vol_targeted_growth_broker_position_comparison_design_summary.csv", ["summary_name", "summary_value"], [{"summary_name": "final_design_status", "summary_value": "vol_targeted_growth_broker_position_comparison_design_ready_manual_review_required"}])
        result = generate_vol_targeted_growth_portfolio_risk_review(root)
        if summary_value(result.summary_rows, "final_risk_review_status") != FINAL_STATUS:
            failures.append("fixture did not produce expected risk review status")
        ...
        for row in result.summary_rows + result.review_rows + result.evidence_rows + result.blocker_rows:
            ...
            for flag in TRUE_FLAGS:
                if str(row.get(flag, "")).lower() != "true":
                    ...
                    return
        code, lines = show_vol_targeted_growth_portfolio_risk_review(root)
        if code != 0 or FINAL_STATUS not in "\n".join(lines):
            failures.append("show command did not display saved risk review")
