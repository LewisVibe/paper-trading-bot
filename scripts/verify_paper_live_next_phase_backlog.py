...
BOT = ROOT / "bot.py"
MODULE = ROOT / "trading_bot" / "research" / "paper_live_next_phase_backlog.py"
...
REQUIRED_MODULE_TOKENS = [
    ...
    "future_review_only",
    "high_growth_sleeve",
    "concentration_drawdown_attribution_reviews_incomplete",
    "crypto_sleeve",
...
def main() -> int:
    ...
    if failures:
        print("Paper-live next-phase backlog verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("Paper-live next-phase backlog verification passed.")
    print("Verified report-only backlog, false approvals, ignored outputs, and no broker/order/config/scheduling calls.")
    return 0
...
def verify_commands(bot_source: str, failures: list[str]) -> None:
    ...
    branches = {
        "--paper-live-next-phase-backlog": 'if sys.argv[1:] == ["--paper-live-next-phase-backlog"]:',
        "--show-paper-live-next-phase-backlog": 'if sys.argv[1:] == ["--show-paper-live-next-phase-backlog"]:',
    }
...
def verify_module(module_source: str, failures: list[str]) -> None:
    if not MODULE.exists():
        failures.append("paper-live next-phase backlog module is missing")
...
def verify_report_output_from_fixture(failures: list[str]) -> None:
    from trading_bot.research.paper_live_next_phase_backlog import (  # noqa: PLC0415
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
        result = generate_paper_live_next_phase_backlog(root)
        status_code, lines = show_paper_live_next_phase_backlog(root)
    ...
    items = {row.get("backlog_item") for row in report_rows}
    for item in REQUIRED_ITEMS:
        if item not in items:
            failures.append(f"report missing backlog item: {item}")
    ...
    if "execution_not_approved" not in by_item.get("crypto_sleeve", {}).get("blocker", ""):
        failures.append("crypto backlog item must preserve crypto execution not approved")
    if "order-capable" not in by_item.get("monitoring_hermes", {}).get("required_before_next_stage", ""):
        failures.append("monitoring/Hermes backlog item must keep order-capable commands never scheduled")
