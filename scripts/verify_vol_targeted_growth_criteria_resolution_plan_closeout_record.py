"""Verify report-only criteria_resolution_plan_open closeout checkpoints."""
...
from trading_bot.research.vol_targeted_growth_criteria_resolution_plan_closeout_approval_wording import (  # noqa: E402
    APPROVAL_PHRASE,
    FINAL_DECISION as WORDING_DECISION,
    generate_vol_targeted_growth_criteria_resolution_plan_closeout_approval_wording,
    show_vol_targeted_growth_criteria_resolution_plan_closeout_approval_wording,
)
from trading_bot.research.vol_targeted_growth_criteria_resolution_plan_closeout_record import (  # noqa: E402
    FINAL_DECISION as RECORD_DECISION,
    generate_vol_targeted_growth_criteria_resolution_plan_closeout_record,
    show_vol_targeted_growth_criteria_resolution_plan_closeout_record,
)
...
def main() -> int:
    ...
    if failures:
        print("Criteria resolution-plan closeout verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Criteria resolution-plan closeout verification passed.")
...
def verify_source_safety(failures: list[str]) -> None:
    for rel in [
        "trading_bot/research/vol_targeted_growth_criteria_resolution_plan_closeout_approval_wording.py",
        "trading_bot/research/vol_targeted_growth_criteria_resolution_plan_closeout_record.py",
    ]:
...
def verify_fixture_outputs(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        seed_inputs(root)
        wording = generate_vol_targeted_growth_criteria_resolution_plan_closeout_approval_wording(root)
        wording_code, wording_lines = show_vol_targeted_growth_criteria_resolution_plan_closeout_approval_wording(root)
        record = generate_vol_targeted_growth_criteria_resolution_plan_closeout_record(root)
        record_code, record_lines = show_vol_targeted_growth_criteria_resolution_plan_closeout_record(root)
        if wording_code != 0:
        ...
        if summary_value(record.summary_rows, "criteria_resolution_plan_open_closed") != "True":
            failures.append("record should mark criteria_resolution_plan_open_closed=True")
        if summary_value(record.summary_rows, "closed_blocker_count") != "2":
            failures.append("record should count two closed blockers")
...
def seed_inputs(root: Path) -> None:
    ...
    data.mkdir()
    write_summary(
        data / "vol_targeted_growth_executable_ticket_criteria_resolution_plan_closeout_candidate_review_summary.csv",
        {
            "final_candidate_review_status": "vol_targeted_growth_criteria_resolution_plan_closeout_candidate_review_manual_review_required",
            "final_candidate_review_decision": "CLOSEOUT_CANDIDATE_NOT_READY_STILL_OPEN",
        },
    )
    write_summary(
