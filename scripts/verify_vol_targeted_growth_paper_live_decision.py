...
from trading_bot.research.vol_targeted_growth_paper_live_decision import (  # noqa: E402
    FINAL_STATUS,
    OUTPUT_FILES,
    SAFETY_FLAGS,
...
COMMANDS = ["--vol-targeted-growth-paper-live-decision", "--show-vol-targeted-growth-paper-live-decision"]
...
def main() -> int:
    ...
    module_source = read_text(ROOT / "trading_bot" / "research" / "vol_targeted_growth_paper_live_decision.py")
    verify_commands(bot_source, failures)
    verify_outputs_ignored(failures)
    verify_source(module_source, failures)
    verify_fixture(failures)
    if failures:
        print("Volatility-targeted growth paper-live decision verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Volatility-targeted growth paper-live decision verification passed.")
    return 0
...
def verify_source(source: str, failures: list[str]) -> None:
    for token in [
        ...
        "paper_live_policy_not_enforced_and_broker_comparison_not_run",
        "manual_review_before_any_readonly_broker_position_comparison",
        "execution_approved",
        "scheduling_approved",
    ]:
    ...
    show_body = source_slice(source, "def show_vol_targeted_growth_paper_live_decision", "def build_decision_rows")
    if "write_rows" in show_body or "generate_vol_targeted_growth_paper_live_decision" in show_body:
...
def verify_fixture(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write_csv(
            data / "vol_targeted_growth_portfolio_risk_review_summary.csv",
            ["summary_name", "summary_value"],
            [{"summary_name": "final_risk_review_status", "summary_value": "vol_targeted_growth_portfolio_risk_manual_review_required"}],
        )
        write_csv(
            data / "vol_targeted_growth_broker_position_comparison_design_summary.csv",
            ["summary_name", "summary_value"],
            [{"summary_name": "final_design_status", "summary_value": "vol_targeted_growth_broker_position_comparison_design_ready_manual_review_required"}],
        )
        write_csv(
        ...
        result = generate_vol_targeted_growth_paper_live_decision(root)
        if summary_value(result.summary_rows, "final_decision_status") != FINAL_STATUS:
            failures.append("fixture did not produce expected decision status")
        if summary_value(result.summary_rows, "research_status") != "research_only_retained":
            failures.append("candidate must remain research-only")
        if summary_value(result.summary_rows, "broker_position_comparison_discussion_status") != "broker_position_comparison_discussion_ready_manual_review_required":
            failures.append("broker comparison should be discussion-ready only")
        if summary_value(result.summary_rows, "paper_live_discussion_status") != "paper_live_discussion_not_approved_research_only":
            failures.append("paper-live discussion must remain not approved")
        for row in result.summary_rows + result.decision_rows + result.evidence_rows + result.blocker_rows:
            ...
            for flag in TRUE_FLAGS:
                if str(row.get(flag, "")).lower() != "true":
                    ...
                    return
        code, lines = show_vol_targeted_growth_paper_live_decision(root)
        if code != 0 or FINAL_STATUS not in "\n".join(lines):
            failures.append("show command did not display saved paper-live decision")
