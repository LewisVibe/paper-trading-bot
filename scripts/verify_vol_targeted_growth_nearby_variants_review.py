...
from trading_bot.research.vol_targeted_growth_nearby_variants_review import (  # noqa: E402
    FINAL_STATUS,
    OUTPUT_FILES,
    PREFERRED_CANDIDATE,
    PREVIEW_STATUS,
...
EXPECTED_OUTPUTS = [
    ...
    "data/vol_targeted_growth_nearby_variants_evidence.csv",
    "data/vol_targeted_growth_nearby_variants_blockers.csv",
]
...
def main() -> int:
    ...
    module_source = read_text(ROOT / "trading_bot" / "research" / "vol_targeted_growth_nearby_variants_review.py")
    ...
    if failures:
        print("Volatility-targeted growth nearby-variants review verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("Volatility-targeted growth nearby-variants review verification passed.")
    print("Verified saved-output nearby-variant review, false approvals, and no broker/order/config/scheduling paths.")
    return 0
...
def verify_command_registration(bot_source: str, failures: list[str]) -> None:
    ...
    for token in ["generate_vol_targeted_growth_nearby_variants_review", "show_vol_targeted_growth_nearby_variants_review"]:
...
def verify_source_boundaries(module_source: str, failures: list[str]) -> None:
    required = [
        PREFERRED_CANDIDATE,
        "preferred_variant_retained_manual_review_required",
        "higher_return_higher_drawdown_variant_manual_review_required",
        "preferred_15_20_retains_best_calmar_and_sharpe_but_requires_manual_review",
        PREVIEW_STATUS,
        "manual_review_15_20_vs_20_20_and_25_20_before_preview_design_decision",
        "preview_candidate_approved",
        "preview_implementation_approved",
        "execution_approved",
        "paper_execution_approved",
    ...
    show_body = source_slice(module_source, "def show_vol_targeted_growth_nearby_variants_review", "def build_review_rows")
    if "write_rows" in show_body or "generate_vol_targeted_growth_nearby_variants_review" in show_body:
...
def verify_fixture_generation(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write_csv(
            ...
            [
                row(PREFERRED_CANDIDATE, "0.15", "20", "19.0011", "1.2861", "-18.1016", "1.0497", "22.0", "1.2", "1.1"),
                row("higher_growth_multi_sleeve_target_vol_10_win_20_cap_1x", "0.1", "20", "13.7208", "1.2787", "-13.1119", "1.0464", "16.0", "1.1", "1.0"),
                row("higher_growth_multi_sleeve_target_vol_20_win_20_cap_1x", "0.2", "20", "21.5042", "1.2511", "-21.0717", "1.0205", "24.0", "1.1", "1.0"),
                row("higher_growth_multi_sleeve_target_vol_15_win_60_cap_1x", "0.15", "60", "18.1077", "1.2072", "-18.5157", "0.9780", "21.0", "1.0", "0.9"),
            ],
        )
        ...
        result = generate_vol_targeted_growth_nearby_variants_review(root)
        status = summary_value(result.summary_rows, "final_nearby_review_status")
        if status != FINAL_STATUS:
            failures.append(f"fixture should produce nearby manual-review status, got {status}")
        if summary_value(result.summary_rows, "best_calmar_variant").split(":")[0] != PREFERRED_CANDIDATE:
            failures.append("preferred candidate should remain best Calmar in fixture")
        if summary_value(result.summary_rows, "best_sharpe_variant").split(":")[0] != PREFERRED_CANDIDATE:
            failures.append("preferred candidate should remain best Sharpe in fixture")
        if summary_value(result.summary_rows, "preview_status") != PREVIEW_STATUS:
            failures.append("preview status must remain blocked")
        if not any(row.get("variant_status") == "higher_return_higher_drawdown_variant_manual_review_required" for row in result.review_rows):
            failures.append("fixture should flag higher-return/higher-drawdown neighboring variant")

        for collection in [result.review_rows, result.summary_rows, result.evidence_rows, result.blocker_rows]:
            for output_row in collection:
                ...
                for flag in TRUE_FLAGS:
                    if str(output_row.get(flag, "")).lower() != "true":
                        ...
                        return
        code, lines = show_vol_targeted_growth_nearby_variants_review(root)
        display = "\n".join(lines)
        if code != 0:
            failures.append(f"show command should succeed after generation, got {code}")
        for token in [FINAL_STATUS, PREFERRED_CANDIDATE, "execution_approved=false", "scheduling_approved=false"]:
            if token not in display:
                failures.append(f"display output missing token: {token}")


def row(candidate_name: str, target_vol: str, window: str, cagr: str, sharpe: str, max_dd: str, calmar: str, oos_cagr: str, oos_sharpe: str, oos_calmar: str) -> dict[str, str]:
    return {
