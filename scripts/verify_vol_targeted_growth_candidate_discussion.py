...
from trading_bot.research.vol_targeted_growth_candidate_discussion import (  # noqa: E402
    FINAL_STATUS,
    OUTPUT_FILES,
    SAFETY_FLAGS,
...
COMMANDS = ["--vol-targeted-growth-candidate-discussion", "--show-vol-targeted-growth-candidate-discussion"]
...
TRUE_FLAGS = [
    ...
    "saved_output_only",
    "manual_review_only",
    "candidate_discussion_only",
    "proposal_only",
    "preview_only",
    "vol_targeted_candidate_proposal_ready",
...
def main() -> int:
    ...
    module_source = read_text(ROOT / "trading_bot" / "research" / "vol_targeted_growth_candidate_discussion.py")
    verify_commands(bot_source, failures)
    verify_outputs_ignored(failures)
    verify_source(module_source, failures)
    verify_fixture(failures)
    if failures:
        print("Volatility-targeted growth candidate discussion verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Volatility-targeted growth candidate discussion verification passed.")
    return 0
...
def verify_source(source: str, failures: list[str]) -> None:
    for token in [
        FINAL_STATUS,
        "non_executable_paper_live_candidate_proposal_ready_manual_review_required",
        "qqq100_retained_as_incumbent_paper_live_seed",
        "proposal_ready_but_not_implemented_or_approved",
        "paper_live_candidate_approved",
        "preview_implementation_approved",
        "order_instructions_created",
        "execution_approved",
    ...
    show_body = source_slice(source, "def show_vol_targeted_growth_candidate_discussion", "def determine_final_status")
    if "write_rows" in show_body or "generate_vol_targeted_growth_candidate_discussion" in show_body:
...
def verify_fixture(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        seed_inputs(root)
        result = generate_vol_targeted_growth_candidate_discussion(root)
        if summary_value(result.summary_rows, "final_candidate_discussion_status") != FINAL_STATUS:
            failures.append("fixture did not produce expected candidate discussion status")
        if summary_value(result.summary_rows, "proposal_status") != "non_executable_paper_live_candidate_proposal_ready_manual_review_required":
            failures.append("fixture should produce proposal-ready but non-executable status")
        if summary_value(result.summary_rows, "qqq100_status") != "qqq100_retained_as_incumbent_paper_live_seed":
            failures.append("QQQ100 should remain incumbent seed")
        if summary_value(result.summary_rows, "largest_blocker") != "proposal_ready_but_not_implemented_or_approved":
            failures.append("largest blocker should keep proposal unimplemented/unapproved")
        for row in result.summary_rows + result.discussion_rows + result.evidence_rows + result.blocker_rows:
            for flag in FALSE_FLAGS:
            ...
            for flag in TRUE_FLAGS:
                if str(row.get(flag, "")).lower() != "true":
                    ...
                    return
        code, lines = show_vol_targeted_growth_candidate_discussion(root)
        if code != 0 or FINAL_STATUS not in "\n".join(lines):
            failures.append("show command did not display saved candidate discussion")
...
def seed_inputs(root: Path) -> None:
    ...
    write_csv(data / "vol_targeted_growth_gate_review_blockers.csv", ["blocker_name"], [{"blocker_name": "paper_live_candidate_not_approved"}])
    write_csv(
        data / "vol_targeted_growth_stricter_paper_live_gate_design_summary.csv",
        ["summary_name", "summary_value"],
        [{"summary_name": "final_gate_design_status", "summary_value": "vol_targeted_growth_stricter_paper_live_gate_design_ready_manual_review_required"}],
    )
    write_csv(
    ...
    write_csv(
        ...
        ["summary_name", "summary_value"],
        [{"summary_name": "final_preview_signal_status", "summary_value": "vol_targeted_growth_preview_signal_saved_report_only"}],
    )
    write_csv(
