"""Verify saved-price snapshot run approval remains price-only and non-executing."""
...
from trading_bot.research.vol_targeted_growth_saved_price_snapshot_run_approval import (  # noqa: E402
    RECORD_DECISION,
    RECORD_OUTPUTS,
    WORDING_OUTPUTS,
    generate_vol_targeted_growth_saved_price_snapshot_run_approval_record,
    generate_vol_targeted_growth_saved_price_snapshot_run_approval_wording,
    show_vol_targeted_growth_saved_price_snapshot_run_approval_record,
)
...
def main() -> int:
    ...
    if failures:
        print("Saved-price snapshot run approval verification failed.")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("Saved-price snapshot run approval verification passed.")
    print("Verified future run approval only, no price fetches, no quantities, false execution approvals, and no broker/order/scheduling calls.")
    return 0
...
def verify_source_safety(failures: list[str]) -> None:
    source = (ROOT / "trading_bot/research/vol_targeted_growth_saved_price_snapshot_run_approval.py").read_text(encoding="utf-8")
    for token in [
        "TradingClient(",
        "MarketOrderRequest(",
        "submit_order",
...
def verify_fixture_outputs(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        seed_inputs(root)
        wording = generate_vol_targeted_growth_saved_price_snapshot_run_approval_wording(root)
        record = generate_vol_targeted_growth_saved_price_snapshot_run_approval_record(root)
        code, lines = show_vol_targeted_growth_saved_price_snapshot_run_approval_record(root)
        if code != 0:
            failures.append("record display failed after generation")
        output = "\n".join(wording.summary_lines + record.summary_lines + lines)
        for phrase in [
            "SAVED_PRICE_SNAPSHOT_RUN_APPROVAL_WORDING_DEFINED_NOT_RECORDED",
            RECORD_DECISION,
            "saved_price_snapshot_run_approved=True",
        ...
        if summary_value(record.summary_rows, "saved_price_snapshot_run_approval_recorded") != "True":
            failures.append("record should mark saved-price snapshot run approval recorded")
        if summary_value(record.summary_rows, "saved_price_snapshot_run_approved") != "True":
            failures.append("record should approve only a future price snapshot run")
